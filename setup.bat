...
node --experimental-strip-types tools/npm/npm.mts install
if errorlevel 1 (
...
echo.
echo [4/4] 编译构建项目...
node --experimental-strip-types tools/npm/npm.mts run build
if errorlevel 1 (
...
echo  开发启动指令: node --experimental-strip-types tools/npm/npm.mts run dev
echo  测试执行指令: node --experimental-strip-types tools/npm/npm.mts test
echo ========================================================
pause

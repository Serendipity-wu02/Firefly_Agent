...
export interface SpeakingMotionOptions {
  ...
  restoreParameterIds?: readonly string[];
}
...
export class SpeakingMotionController {
  ...
  private readonly motionName: string;
  private readonly fallbackIndex: number;
  private readonly restoreParameterIds: readonly string[];

  constructor(model: Live2DModel, options: SpeakingMotionOptions = {}) {
    this.model = model;
    this.group = options.group ?? SWING_GROUP;
    this.motionName = options.motionName ?? SWING_MOTION_NAME;
    this.fallbackIndex = options.fallbackIndex ?? FALLBACK_SWING_INDEX;
    this.restoreParameterIds = options.restoreParameterIds ?? SWING_PARAMS;
  }
  ...
  stop(): void {
    ...
    try {
      ...
      console.warn("[Firefly] stop speaking motion failed", err);
    }
    this.attachRestore();
  }
  ...
  private async playSwingMotion(): Promise<void> {
    ...
    try {
      ...
      await this.model.motion(this.group, index, MotionPriority.IDLE);
    } catch (err) {
      console.warn("[Firefly] speaking motion failed", err);
    }
  }

  private findSwingIndex(): number {
    const internal = this.model.internalModel as unknown as InternalModelLike;
    const defs = internal.motionManager.definitions?.[this.group];
    if (!Array.isArray(defs) || defs.length === 0) return -1;
    const byName = defs.findIndex((def) => def?.Name === this.motionName);
    if (byName >= 0) return byName;
    return this.fallbackIndex < defs.length ? this.fallbackIndex : -1;
  }
  ...
  private attachRestore(): void {
    ...
    this.restoreFrom = this.restoreParameterIds.map((id) => {
      try {
    ...
    this.restoreHook = () => {
      ...
      this.restoreParameterIds.forEach((id, i) => {
        try {

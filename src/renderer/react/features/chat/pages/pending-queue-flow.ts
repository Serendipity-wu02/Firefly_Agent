...
export function createPendingQueueFlow(getHost: () => PendingQueueFlowHost): PendingQueueFlow {
  ...
  const consumingSessions = new Set<string>();
  const consumeAfterRun = new Map<string, ConversationMode>();
  ...
  async function enqueue(
    ...
    try {
      result = await store.pendingEnqueue(sessionId, {
        ...
        ...(entry.requireDocumentRead ? { requireDocumentRead: true } : {}),
    ...
    if (!result.ok) {
      rememberFailedEnqueue(cacheKey, id);
      if (notifyError && !("canceled" in result && result.canceled)) host.reportError(t("chatPage.errorEnqueueFailed", { error: result.error }));
      return false;
    }
    failedEnqueueIds.delete(cacheKey);
    if (consumingSessions.has(sessionId)) consumeAfterRun.set(sessionId, mode);
  ...
  async function consume(mode: ConversationMode, sessionId: string): Promise<void> {
    if (consumingSessions.has(sessionId)) return;
    consumingSessions.add(sessionId);
    try {
      await consumeCurrent(mode, sessionId);
    } finally {
      consumingSessions.delete(sessionId);
      const nextMode = consumeAfterRun.get(sessionId);
      if (nextMode) {
        consumeAfterRun.delete(sessionId);
        void consume(nextMode, sessionId);
      }
    }
  }

  async function consumeCurrent(mode: ConversationMode, sessionId: string): Promise<void> {
    const host = getHost();
    const store = host.getStore();
    if (!store) return;
  ...
  function handleRunFinished(input: { mode: ConversationMode; sessionId: string; queuePaused: boolean }): void {
    ...
    if (input.queuePaused) {
      consumeAfterRun.delete(input.sessionId);
      return;
    }
    if (consumingSessions.has(input.sessionId)) {
      consumeAfterRun.set(input.sessionId, input.mode);
    } else {

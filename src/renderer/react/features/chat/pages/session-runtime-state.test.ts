...
describe("session runtime presentation state", () => {
  ...
  it("settles the displayed composer when mode state is idle even if a ref render is stale", () => {
    const activeRuns = { "session-a": { runId: "run-a" } };

    expect(isCurrentRunBusy(activeRuns, "session-a", true)).toBe(true);
    expect(isCurrentRunBusy(activeRuns, "session-a", false)).toBe(false);
    expect(isCurrentRunBusy(activeRuns, "session-b", true)).toBe(false);
    expect(isCurrentRunBusy(activeRuns, undefined, true)).toBe(false);
  });

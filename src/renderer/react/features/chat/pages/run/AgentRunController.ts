...
export class AgentRunController {
  ...
  private workReadReport: WorkReadReport | undefined;

  private ensureWorkReadReport(): void {
    if (this.workReadReport || this.input.targetMode !== "work") return;
    const userMessage = this.input.session.messages.find((message) => message.id === this.input.userMessageId);
    const scopes = (userMessage?.attachments ?? []).flatMap((attachment) =>
      attachment.kind === "document" && attachment.readScope ? [attachment.readScope] : [],
    );
    ...
    this.workReadReport = {
      ...
      files: scopes.map((scope) => ({
        ...
        status: "unverified",
        coveredLines: 0,
        requiredLines: scope.endLine,
        totalLines: scope.totalLines,
      })),
    };
    this.deps.host.patchMessage(this.input.sessionId, this.input.assistantId, { workReadReport: this.workReadReport });
  }
  ...
  async start(): Promise<void> {
    ...
    const existingRun = this.deps.registries.activeRuns.current[this.input.sessionId];
    if (!existingRun) {
    ...
    const checkpointTrigger = (status: "running" | "waiting_user" | "terminal") => {
    ...
    try {
      ...
      if (existingRun) this.deps.host.setModeBusy(this.input.targetMode, true);
      this.deps.registries.activeRuns.current = {
        ...this.deps.registries.activeRuns.current,
        [this.input.sessionId]: {
          assistantId: this.input.assistantId,
      ...
      this.deps.host.patchMessage(this.input.sessionId, this.input.assistantId, {
        ...
        runStage: resolveRunFinishedStage({ status: this.terminalStatus }),
      ...
      if (this.deps.registries.checkpointTriggers.current[this.input.sessionId] === checkpointTrigger) {
      ...
      if (currentActive?.assistantId === this.input.assistantId) {
        ...
        this.deps.host.setModeBusy(
          this.input.targetMode,
          Object.values(nextActive).some((run) => run.mode === this.input.targetMode),
        );
  ...
  private buildCheckpoint(status: "running" | "waiting_user" | "terminal"): ChatMessage {
    return {
      ...
      workReadReport: this.workReadReport,
      runSnapshot: {
  ...
  private handleEvent(event: AguiEvent) {
    ...
    if (event.type === "REASONING_MESSAGE_START") {
      ...
      if (report && Array.isArray(report.files) && ["complete", "partial", "missing", "changed", "unverified"].includes(report.status)) {
        ...
        this.deps.host.patchMessage(this.input.sessionId, this.input.assistantId, { workReadReport: report });
      }

...
function createHarness(startRun?: (input: Record<string, unknown>) => Promise<void>): FlowHarness {
  ...
  const host: PendingQueueFlowHost = {
    ...
    startRun: async (input) => {
      ...
      await startRun?.(input as unknown as Record<string, unknown>);
    },
    reportError: (message) => {
...
describe("页面待发队列流程", () => {
  it("新会话的空队列检查与入队交错时，重新消费并只启动一次运行", async () => {
    let finishInitialRead!: (session: ChatSession) => void;
    const initialRead = new Promise<ChatSession>((resolve) => { finishInitialRead = resolve; });
    const { flow, store, calls } = createHarness();
    store.get.mockImplementationOnce(() => initialRead).mockResolvedValue(makeSession());
    store.pendingClaim
      .mockResolvedValueOnce({ ok: true, claimed: false })
      .mockResolvedValueOnce(makeClaimed());

    const initialConsume = flow.consume("work", "s1");
    expect(await flow.enqueue("s1", "work", {
      id: "q-1",
      rawContent: "读取公开文件",
      visibleContent: "读取公开文件",
    })).toBe(true);
    await flow.consume("work", "s1");
    finishInitialRead(makeSession());
    await initialConsume;
    await flushAsync();

    expect(store.pendingClaim).toHaveBeenCalledTimes(2);
    expect(calls.runs).toHaveLength(1);
    expect(calls.runs[0]).toMatchObject({ sessionId: "s1", userMessageId: "q-1" });
  });

  it("同一会话的两个消费入口只恢复一次认领并启动一次运行", async () => {
    const { flow, store, calls } = createHarness();
    ...
    await Promise.all([flow.consume("chat", "s1"), flow.consume("chat", "s1")]);
  ...
  it("运行结束后消费下一条，同会话重复触发不重启当前运行", async () => {
    let finishFirstRun!: () => void;
    const firstRun = new Promise<void>((resolve) => { finishFirstRun = resolve; });
    const { flow, store, calls } = createHarness(async (input) => {
      if (input.userMessageId === "q-1") await firstRun;
    });
    store.get.mockResolvedValue(makeSession());
    store.pendingClaim
      .mockResolvedValueOnce(makeClaimed())
      .mockResolvedValueOnce(makeClaimed({
        userMessage: { id: "q-2", role: "user", content: "第二条", at: 2 },
        visibleContent: "第二条",
      }));
    ...
    expect(calls.runs.map((run) => run.userMessageId)).toEqual(["q-1"]);
    ...
    expect(calls.runs.map((run) => run.userMessageId)).toEqual(["q-1", "q-2"]);
    expect(store.pendingClaim).toHaveBeenCalledTimes(2);
  });

  it("启动被拒后暂停队列，不因并发消费重试同一认领", async () => {
    let finishRun!: () => void;
    const running = new Promise<void>((resolve) => { finishRun = resolve; });
    const { flow, store, calls } = createHarness(async () => { await running; });
    store.get.mockResolvedValue(makeSession({
      pendingDispatch: { messageId: "q-1", claimedAt: 1 },
      messages: [{ id: "q-1", role: "user", content: "排队消息", at: 1 } as ChatMessage],
    }));

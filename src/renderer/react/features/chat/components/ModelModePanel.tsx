...
function getProviderIconUrl(provider: string | undefined): string | null {
...
export function ModelModePanel() {
  ...
  const reload = useCallback(async () => {
    ...
    if (!catalog) throw new Error("MODEL_CATALOG_UNAVAILABLE");
  ...
  const handleSetDefault = useCallback((id: string) => {
    void api()
      ...
      .then(reload).catch(() => setLoadFailed(true));
  }, [reload]);
  ...
  return (
    <div className="model-panel">
      ...
      {loading ? (
        ...
        <div className="model-panel__empty" role="alert">{t("modelPanel.loadFailed")}</div>
      ) : (

...
async function mountComposer(options: MountOptions = {}) {
  ...
  await act(async () => {});
  return async (modelBusy: boolean) => {
    options.modelBusy = modelBusy;
    await act(async () => {
      root!.render(createElement(Harness));
    });
  };
...
describe("ChatComposer 忙闲路由（真实 Sender）", () => {
  it("Work 终态更新无需编辑或切会话即可恢复空闲提示与发送路由", async () => {
    const setBusy = await mountComposer({ modelBusy: true, mode: "work", workspaceName: "public-fixtures" });
    const editor = textarea();
    expect(editor.placeholder).toBe(t("composer.placeholderBusy"));
    expect(buttonByLabel(t("composer.stopRun"))).toBeTruthy();

    await setBusy(false);

    expect(textarea()).toBe(editor);
    expect(editor.placeholder).toBe(t("composer.placeholderTask"));
    expect(editor.disabled).toBe(false);
    expect([...host!.querySelectorAll("button")].some((button) => button.getAttribute("aria-label") === t("composer.stopRun"))).toBe(false);
    input("下一条公开任务");
    pressEnter();
    expect(handlers.onSubmit).toHaveBeenCalledExactlyOnceWith("下一条公开任务");

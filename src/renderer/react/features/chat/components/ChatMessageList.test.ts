...
describe("formal answer visibility", () => {
  it("shows Main file-read status separately from the model answer", () => {
    const message: ChatMessageItem = {
      ...
      workReadReport: { status: "partial", files: [{ name: "public.txt", status: "partial", coveredLines: 2, requiredLines: 4, totalLines: 4 }] },
    };
    const items = createMessageItems([message], []);
    expect(items.map((item) => item.role)).toEqual(["assistant", "system"]);
    expect(String(items[1].content)).toContain("2/4");
    expect(message.content).toBe("我已读完全文");
  });
...
describe("function-calling round presentation", () => {
  it("renders one collapsible activity group per model round with reasoning inside", () => {
    ...
    expect(html).not.toContain("cy-agent-round__art-image");
    expect(html).not.toContain("cy-agent-round__status");
    expect(html).toContain("cy-reasoning-status-art is-thinking");
    expect(html).not.toContain("status-moods");
    expect(html).toContain("流萤已完成 · 浏览 1 个目录");
    expect(html).toContain("流萤正在读取文件");
    expect(html).toContain("先看项目结构");

...
export function ModelSelector({ activeProfileId, onSelect }: { activeProfileId?: string; onSelect: (id: string) => void }) {
  ...
  return (
    <Popover open={open} onOpenChange={(next) => { setOpen(next); if (next) void load(); }} trigger="click" placement="topLeft"
      content={<div className="cy-model-selector__menu">{loadState === "loading" ? <span>{t("common.loading")}</span> : loadState === "error" ? <span role="alert">{t("modelPanel.loadFailed")}</span> : profiles.length ? profiles.map((profile) => <button type="button" key={profile.id} onClick={() => { onSelect(profile.id); setOpen(false); }}><strong>{profile.displayName || profile.provider}</strong><small>{profile.model}</small></button>) : <span>{t("modelSelector.emptyHint")}</span>}</div>}>

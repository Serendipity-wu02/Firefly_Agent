...
describe("work run presentation", () => {
  ...
  it("keeps internal routing out of the user-facing stage copy", () => {
    expect(describeRunStage({ kind: "understanding" })).toBe("流萤正在理解需求…");
    expect(describeRunStage({ kind: "planning" })).toBe("流萤正在规划任务…");
    expect(describeRunStage({ kind: "executing", detail: "查询淄博天气" }))
      .toBe("流萤正在执行：查询淄博天气…");
    expect(describeRunStage({ kind: "waiting_permission" })).toBe("流萤正在获取审批…");
    expect(describeRunStage({ kind: "waiting_user" })).toBe("流萤正在询问…");
    expect(describeRunStage({ kind: "responding" })).toBe("流萤正在组织回复…");
  });

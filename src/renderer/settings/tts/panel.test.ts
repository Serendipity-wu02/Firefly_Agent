...
describe("TTS settings panel", () => {
  ...
  it("shows a warning notice instead of window.alert when GPT-SoVITS fields are empty", async () => {
    ...
    expect(notice!.textContent).toContain("请先填写 GPT-SoVITS API 地址");
    expect((document.getElementById("tts-gptsovits-url") as HTMLInputElement).value).toBe("");
    expect((document.getElementById("tts-gptsovits-prompt-text") as HTMLInputElement).value).toBe("");

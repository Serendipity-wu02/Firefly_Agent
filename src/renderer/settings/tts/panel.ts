...
const TTS_TEST_TEXT = "你好，我是流萤，很高兴见到你。";
...
async function loadTtsConfig(): Promise<void> {
  ...
  // GPT-SoVITS
  ttsEl("tts-gptsovits-url").value = String(ttsState.config.ttsGptsovitsBaseUrl ?? "");
...
document.getElementById("tts-mimo-test")?.addEventListener("click", async () => {
  ...
  if (!voiceAudioPath) { showNotice({ tone: "warning", message: "请先选择流萤克隆参考音频" }); return; }

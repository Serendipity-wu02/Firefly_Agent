...
export interface DocumentMessageAttachment {
  ...
  chunks?: number;
  reason?: string;
  readScope?: WorkReadScope;
}

export interface WorkReadScope {
  name: string;
  path: string;
  sha256: string;
  totalLines: number;
...
export interface WorkReadReport {
  status: "complete" | "partial" | "missing" | "changed" | "unverified";
  files: Array<{ name: string; status: "complete" | "partial" | "missing" | "changed" | "unverified"; coveredLines: number; requiredLines: number; totalLines: number }>;
}

...
describe("MiniMax 音色 ID", () => {
  it("拒绝不符合官方规则的音色 ID", () => {
    expect(validateMiniMaxVoiceId("firefly")).toContain("8 到 256");
    expect(validateMiniMaxVoiceId("1firefly_voice")).toContain("英文字母");
    expect(validateMiniMaxVoiceId("firefly_voice_")).toContain("不能是 - 或 _");
    expect(validateMiniMaxVoiceId("firefly voice")).toContain("仅能包含");
  });

  it("生成符合官方规则且带随机后缀的建议 ID", () => {
    const voiceId = createUniqueMiniMaxVoiceId(() => 0.123456789, new Date("2026-08-28T10:20:30Z"));

    expect(voiceId).toMatch(/^firefly-voice-20260828-102030-[a-z0-9]+$/);

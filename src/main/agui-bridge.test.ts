...
describe("agui-bridge sticker event ordering", () => {
  ...
  it("rejects a changed required file before model dispatch", async () => {
    ...
    try {
      ...
      fs.writeFileSync(selected, "before");
      const { inspectWorkReadFile } = await import("./chats/work-read-scope");
      const scope = inspectWorkReadFile(selected);
      fs.writeFileSync(selected, "after");
      mocks.getSession.mockReturnValue({
        id: "work-read-changed", mode: "work",
        workspaceBinding: { workspaceRoot: root, displayName: "public", boundAt: 1 },
        messages: [{ id: "user-read", role: "user", content: "read", at: 1, attachments: [{ kind: "document", name: "public.txt", filePath: selected, status: "pending", readScope: scope }] }],
      });
      const { registerAgUiIpc } = await import("./agui-bridge");
      const buildOptions = vi.fn();
      registerAgUiIpc(buildOptions, async () => {}, () => null);
      const handler = mocks.handlers.get(IPC.AGUI_RUN)!;
      await expect(handler({ sender: { isDestroyed: () => false, send: () => {} } }, {
  ...
  it("passes Main-owned Work read scope into the existing run and emits separate missing-evidence status", async () => {
    ...
    try {
      const selected = path.join(root, "public.txt");
      fs.writeFileSync(selected, "public data");
      const { inspectWorkReadFile } = await import("./chats/work-read-scope");
      const scope = inspectWorkReadFile(selected);
      mocks.getSession.mockReturnValue({
        id: "work-read-valid", mode: "work",
        workspaceBinding: { workspaceRoot: root, displayName: "public", boundAt: 1 },
        messages: [{ id: "user-read", role: "user", content: "read", at: 1, attachments: [{ kind: "document", name: "public.txt", filePath: selected, status: "pending", readScope: scope }] }],
      });
      const { registerAgUiIpc } = await import("./agui-bridge");
      ...
      registerAgUiIpc(buildOptions, async () => ({}), () => null);
      const events: Array<{ type?: string; name?: string; value?: { status?: string } }> = [];
      const handler = mocks.handlers.get(IPC.AGUI_RUN)!;
      await handler({ sender: { isDestroyed: () => false, send: (_channel: string, event: typeof events[number]) => events.push(event) } }, {
        sessionId: "work-read-valid", userTurnId: "user-read", messages: [{ role: "user", content: "read" }],
      });
      await expect.poll(() => events.some((event) => event.type === "RUN_FINISHED")).toBe(true);
      expect(buildOptions).toHaveBeenCalledWith(expect.objectContaining({

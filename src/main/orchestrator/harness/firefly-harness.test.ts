/**
 * FireflyHarness 完成语义测试：模型不再调用工具时立即结束
 *
 * 验收不变量：
...
describe("FireflyHarness completion", () => {
  beforeEach(() => {
  ...
  it("forwards each candidate text chunk before the provider releases the next chunk", async () => {
    ...
    const running = runFireflyHarness({
      systemPrompt: "you are a test agent",
      messages: [{ role: "user", content: "完成任务" }],
      tools: [],
      vendorConfig,
  ...
  it("routes a tool round failure to a unified error terminal settlement", async () => {
    ...
    // 工具轮抛出的非取消错误不得冲出 runFireflyHarness：
...
describe("FireflyHarness run adjustments", () => {
  beforeEach(() => {
    mockedDispatch.mockReset();
    fakeStreamChatWithSdk.mockClear();
    recordUsage.mockReset();
...
describe("FireflyHarness context usage snapshots", () => {
  beforeEach(() => {
    mockedDispatch.mockReset();
    fakeStreamChatWithSdk.mockClear();
    recordUsage.mockReset();
...
describe("FireflyHarness transcript sink", () => {
  beforeEach(() => {
    mockedDispatch.mockReset();
    fakeStreamChatWithSdk.mockClear();
    recordUsage.mockReset();
  ...
  it("awaits assistant persistence before dispatching any tool", async () => {
    ...
    const runPromise = runFireflyHarness(harnessInput({ transcriptSink: sink }));

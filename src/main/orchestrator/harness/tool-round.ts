...
async function commitToolResult(
  ...
  if (call.name === "read_file" && result.outcome === "success" && typeof result.output === "string") {
    try {
      const parsed = JSON.parse(result.output) as Record<string, unknown>;
      if (parsed.success !== false && parsed.truncated === false
        && typeof parsed.path === "string"
        && typeof parsed.canonicalPath === "string" && typeof parsed.sha256 === "string"
        && Number.isInteger(parsed.startLine) && Number.isInteger(parsed.endLine) && Number.isInteger(parsed.totalLines)) {
        fileRead = {
          path: parsed.path,
          canonicalPath: parsed.canonicalPath,
          sha256: parsed.sha256,
          startLine: parsed.startLine as number,
          endLine: parsed.endLine as number,
          totalLines: parsed.totalLines as number,
        };
      }
    } catch {
      fileRead = undefined;

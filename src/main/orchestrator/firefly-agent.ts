...
export interface AgentLoopResult {
   ...
   * FireflyAgent.runWithEvents 据此决定 RUN_FINISHED.result 的形状。
   * 未设置时由 FireflyAgent 通过 completionReason 推断（兼容旧调用方）。
   */
...
export interface FireflyRunOptions {
   ...
   * - 非 bridge 调用方允许不传：FireflyAgent.runWithEvents 会 fallback 生成一次。
   ...
   * - FireflyAgent.runWithEvents 把它连接到内部 abortController（first-source-wins）。
...
export interface FireflyRunResult {
  ...
  executionMode?: AgentExecutionMode;
  socialContext?: FireflyRunOptions["socialContext"];
  /**
   * Canonical 终态结算（exactly-once，见 run-settlement.ts）。
...
const LOG_PREFIX = "[FireflyAgent]";

/**
 * 生成 fallback runId（仅在调用方未通过 FireflyRunOptions.runId 注入时使用）。
...
/**
 * FireflyAgent —— 单次对话一个实例。
 *
 * 用法：
 *   const agent = new FireflyAgent({ threadId });
 *   const result = await agent.runAgentWith(options);  // 跑循环 + 事件流
 */
export class FireflyAgent extends AbstractAgent {
  /** 跑循环结果，run() 完成后可取（供桥层做副作用）。 */
  lastResult?: FireflyRunResult;

  /**
   * 跑 Agent 循环并返回事件流。桥层订阅这个流转发给渲染进程。
   * 传入的 options 会原样跑——settings/messages/timeout 都在这里。
   */
  runWithEvents(options: FireflyRunOptions): Observable<BaseEvent> {
    ...
    const runOptions: FireflyRunOptions = {
      ...options,
      runId,
    ...
    return new Observable<BaseEvent>((subscriber) => {
      ...
      (async () => {
        try {
          ...
          // 改走 FireflyHarness（native function calling）；否则维持无工具 ChatLoop。
          ...
          if (executionMode === "chat" && !chatWithTools) {
            ...
            // Work / Learn / Code 统一通过 FireflyHarness。
  ...
  protected _runOptions?: FireflyRunOptions;
  run(input: RunAgentInput): Observable<BaseEvent> {
    if (!this._runOptions) {
      return new Observable<BaseEvent>((s) => {
        s.error(new Error("FireflyAgent.run 被直接调用，但未设置 _runOptions。请用 runWithEvents。"));
      });
    }
    void input;

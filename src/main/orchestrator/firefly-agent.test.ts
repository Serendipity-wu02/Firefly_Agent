...
describe("FireflyAgent external signal threading", () => {
  beforeEach(() => {
    mockedRunHarnessWithAdapter.mockClear();
  });

  it("threads FireflyRunOptions.signal into harness signal (abort propagates)", async () => {
    const agent = new FireflyAgent({ threadId: "thread-signal-1" });
  ...
  it("emits exactly one terminal (cancelled) when external signal aborts", async () => {
    const agent = new FireflyAgent({ threadId: "thread-signal-2" });
    const externalController = new AbortController();
  ...
  it("cancelled path does not emit final_answer text as '最终回复被取消。'", async () => {
    const agent = new FireflyAgent({ threadId: "thread-signal-3" });
    const externalController = new AbortController();
  ...
  it("removes the external abort listener and does not abort the harness signal after normal completion", async () => {
    ...
    const agent = new FireflyAgent({ threadId: "thread-signal-cleanup" });
    const externalController = new AbortController();
...
describe("FireflyAgent chat tool enhancement branch", () => {
  beforeEach(() => {
    mockedRunHarnessWithAdapter.mockClear();
  ...
  it("routes chat without tools to runChatLoop (unchanged behavior)", async () => {
    const agent = new FireflyAgent({ threadId: "thread-chat-plain" });
    const events: unknown[] = [];
  ...
  it("routes chat with opted-in tools to the harness (native function calling)", async () => {
    const agent = new FireflyAgent({ threadId: "thread-chat-tools" });
    const externalController = new AbortController();
    const events: unknown[] = [];
    const sub = agent.runWithEvents({
      settings: { provider: "test", baseUrl: "", model: "", apiKey: "", contextWindowTokens: 256000 } as never,
...
describe("FireflyAgent transcript sink wiring", () => {
  /** 可断言的假轨迹提交端：默认全部成功。 */
  ...
  it("closes the transcript interruption before emitting the cancelled terminal", async () => {
    const agent = new FireflyAgent({ threadId: "thread-chat-cancel" });
    const externalController = new AbortController();
  ...
  it("reports a runtime error when the ChatLoop interruption closure fails", async () => {
    const agent = new FireflyAgent({ threadId: "thread-chat-cancel-fail" });
    const externalController = new AbortController();

...
import { resolveUserAddress } from "../../shared/user-address";
...
export function buildEnvironmentContext(modelInfo?: ModelInfo, userInfo?: UserInfoContext): string {
  ...
  if (userInfo) {
    lines.push("## 用户信息");
    lines.push("");
    if (userInfo.callPreference?.trim()) {
      lines.push(`- 称呼偏好：${userInfo.callPreference.trim()}（称呼用户时优先用这个）`);
    } else if (userInfo.nickname?.trim()) {
      lines.push(`- 昵称：${userInfo.nickname.trim()}（称呼用户时用这个）`);
    }
    ...
    const preferredAddress = resolveUserAddress(userInfo);

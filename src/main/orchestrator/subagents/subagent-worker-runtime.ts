  ...
  async execute(
    ...
    try {
      ...
      if (result.status !== "completed") {
        const budgetTermination = result.terminationReason.kind === "budget_exhausted"
          ? result.terminationReason
          : undefined;
        return this.failRunning(taskId, {
          code: budgetTermination === undefined ? "RUNTIME_FAILURE" : "BUDGET_EXHAUSTED",
          message: result.error || `SubAgent worker ended with status ${result.status}.`,
          ...(budgetTermination === undefined ? {} : { terminationReason: budgetTermination }),
        });
      }

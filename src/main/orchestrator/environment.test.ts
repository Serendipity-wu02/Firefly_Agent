...
describe("buildEnvironmentContext timezone", () => {
  ...
  it("uses saved nickname before the Firefly default address", () => {
    const nickname = buildEnvironmentContext(undefined, { nickname: "小王", callPreference: "  " });
    expect(nickname).toContain("称呼使用：在重要提问或确认时，可以自然使用一次「小王」");
    expect(nickname).not.toContain("称呼偏好：  ");
    const defaultAddress = buildEnvironmentContext(undefined, { nickname: "", callPreference: "" });
    expect(defaultAddress).toContain("称呼使用：在重要提问或确认时，可以自然使用一次「开拓者」");
  });

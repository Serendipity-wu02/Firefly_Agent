...
describe("TaskRuntime", () => {
  ...
  it("emits the supplied portrait lifecycle while retaining the requested execution profile", async () => {
    const store = createStore();
    ...
    expect(lifecycle).toEqual([
      expect.objectContaining({ nickname: "艾利欧", assetFileName: "艾利欧.png", status: "running" }),
      expect.objectContaining({ nickname: "艾利欧", assetFileName: "艾利欧.png", status: "completed" }),
    ]);
    expect(runHarness).toHaveBeenCalledWith(expect.objectContaining({
      tools: [expect.objectContaining({ id: "read_file" })],
    }));
  ...
  it("runs without a character lifecycle when the roster is empty", async () => {
    const store = createStore();
    const lifecycle: TaskDelegationPresentation[] = [];
    const runHarness = vi.fn(async () => ({
      finalAnswer: "检查完成。", finalState: { todoItems: [], uncertainEffects: [] },
  ...
  it.each([
    ...
    const result = await execute({ description: "检查异常结算", prompt: "执行", subagentType: "general" });

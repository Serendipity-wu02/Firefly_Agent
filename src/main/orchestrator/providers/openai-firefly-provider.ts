...
type ChatToolSchema = NonNullable<ChatCompletionRequest["tools"]>[number];

function schemaPropertyNames(schema: ChatToolSchema | undefined): string {
  const properties = schema?.function.parameters.properties;
  if (properties === null || typeof properties !== "object" || Array.isArray(properties)) return "none";
  return Object.keys(properties as Record<string, unknown>).join(",") || "none";
}
...
export class OpenAiFireflyProvider implements IFireflyLlmProvider {
  ...
  async generateCompletion(
    ...
    if (request.toolChoice !== undefined) {
      bodyPayload.tool_choice = request.toolChoice;
    }

    const browserSchema = request.tools?.find((tool) => tool.function.name === "browser_read");
    emitDiagnosticTrace(
      `[Provider Trace] request provider=${this.id} ` +
      `toolNames=${request.tools?.map((tool) => tool.function.name).join(",") || "none"} ` +
      `browserSchema=${browserSchema ? "yes" : "no"} ` +
      `browserParams=${schemaPropertyNames(browserSchema)} ` +
      `toolChoice=${request.toolChoice?.function.name ?? "auto"} stream=${isStreaming}`,
    );
    ...
    if (isStreaming && response.body) {
      ...
      emitDiagnosticTrace(
        `[Provider Trace] response provider=${this.id} mode=stream ` +
        `toolCalls=${finalRes.toolCalls?.map((call) => call.name).join(",") || "none"} ` +
        `contentLength=${finalRes.content.length}`,
      );
      return {
        message: {
          role: "assistant",
    ...
    let toolCalls: ChatCompletionResponse["message"]["toolCalls"] = undefined;
    ...
    emitDiagnosticTrace(
      `[Provider Trace] response provider=${this.id} mode=non_stream ` +
      `toolCalls=${toolCalls?.map((call) => call.name).join(",") || "none"} ` +

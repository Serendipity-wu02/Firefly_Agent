...
export function processSseChunk(
  ...
  const completeLines = (acc.pendingLine + chunkText).split("\n");
  acc.pendingLine = completeLines.pop() ?? "";
  for (const line of completeLines) {
    processSseLine(line, acc, onDeltaText);
  }
}
...
export function finalizeAccumulator(acc: StreamingAccumulator): {
  ...
  if (acc.pendingLine.length > 0) {
    const finalLine = acc.pendingLine;
    acc.pendingLine = "";
    processSseLine(finalLine, acc);
  }

...
describe("build-options", () => {
  it("injects selected Work paths as untrusted references without pretending to include file contents", async () => {
    const result = await buildAgentRunOptions({
      ...
      workDocuments: [{ name: "public.txt", path: "C:\\workspace\\public.txt", requiredEndLine: 2, totalLines: 4, partialAccepted: true }],
    }, createBuildDeps());
    expect(result.options.soulRuntimeContext).toContain("requiredEndLine");
    expect(result.options.soulRuntimeContext).toContain("不可信元数据");

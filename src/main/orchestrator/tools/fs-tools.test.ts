...
describe("read_file structured output", () => {
  it("enforces Main-confirmed content version and partial line range", async () => {
    const selected = path.join(tmpDir, "public.txt");
    fs.writeFileSync(selected, "one\ntwo\nthree\nfour");
    const scope = { ...inspectWorkReadFile(selected), endLine: 2, partialAccepted: true };
    const tool = vi.mocked(toolRegistry.register).mock.calls.find((call) => call[0].id === "read_file")?.[0];
    expect(tool).toBeDefined();
    const context = { userQuery: "read", workReadScopes: [scope] };
    const limited = JSON.parse(await tool!.execute({ path: selected, maxLines: 500 }, context));
    expect(limited).toMatchObject({ startLine: 1, endLine: 2, scopeLimited: true });
    expect(limited.content).not.toContain("three");
    expect(JSON.parse(await tool!.execute({ path: selected, startLine: 3 }, context))).toMatchObject({ success: false, errorCode: "OUTSIDE_CONFIRMED_RANGE" });
    fs.writeFileSync(selected, "changed\ntwo\nthree\nfour");
    expect(JSON.parse(await tool!.execute({ path: selected }, context))).toMatchObject({ success: false, errorCode: "FILE_CHANGED" });
  });

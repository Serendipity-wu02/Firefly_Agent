...
import type { FireflyToolRegistry } from "../registry/tool-registry";
import { FireflyToolDispatcher } from "../registry/tool-dispatcher";
      ...
      while (true) {
        ...
        try {
          ...
          if (isTransientError(rawResult.output) && attempt <= maxRetries) {
            ...
            const waitResult = await waitForCancellableDelay(delayMs, ctx.signal);

...
import { RagSlot, type RagSlotOptions } from "../orchestrator/context/context-slots";
...
export function resolveKnowledgeDataDir(): string {
  const buildKnowledgeDir = path.join(__dirname, "knowledge");
  const sourceKnowledgeDir = path.join(process.cwd(), "src", "main", "rag", "knowledge");

  if (fs.existsSync(path.join(buildKnowledgeDir, "chunks.json"))) {
    return buildKnowledgeDir;
  }
  if (fs.existsSync(path.join(sourceKnowledgeDir, "chunks.json"))) {
    return sourceKnowledgeDir;
  }
  return buildKnowledgeDir;
}
 ...
 * 2. 负责读取 src/main/rag/knowledge/ 中的离线语料与向量索引；
...
export class KnowledgeCoordinator {
  ...
  constructor(options: KnowledgeCoordinatorOptions = {}) {
    this.knowledgeDataDir = options.knowledgeDataDir || resolveKnowledgeDataDir();

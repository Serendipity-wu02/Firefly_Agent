...
export interface QQMusicDesktopBridgeOptions {
  ...
  /** Delay before the one post-command state read; defaults to 300 ms. */
  postControlObservationDelayMs?: number;
}
...
function resolveGsmtcScript(customPath?: string): string {
  ...
  const scriptPaths = [
    ...
    path.join(process.cwd(), "dist", "main", "main", "music", "scripts", scriptName),
  ];
...
export class QQMusicDesktopBridge extends EventEmitter {
  ...
  private readonly postControlObservationDelayMs: number;
  ...
  constructor(options: QQMusicDesktopBridgeOptions = {}) {
    ...
    this.postControlObservationDelayMs = Math.max(0, options.postControlObservationDelayMs ?? 300);
  }
  ...
  /**
   * Starts a state read after any already-running read has completed. This is
   * used after an external control command so an older polling snapshot cannot
   * be presented as the command's observation.
   */
  async pollAfterControl(): Promise<DesktopBridgeSnapshot> {
    const activePoll = this.activePollPromise;
  ...
  async control(action: GsmtcControlAction, signal?: AbortSignal): Promise<GsmtcRawState> {
    ...
    switch (action) {
      ...
      case "next":
      case "prev":
        // QQ Music may publish the next/previous track asynchronously. Wait for
        // the existing bounded GSMTC read once so callers can distinguish a
        // submitted command from an observed track change. This never retries
        // the control command.
        await new Promise<void>((resolve) => setTimeout(resolve, this.postControlObservationDelayMs));
        if (signal?.aborted) {
          return {
            ...res,
            ok: false,

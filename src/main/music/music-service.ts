...
export type QQMusicControlFailureCode =
  ...
  | "CONTROL_UNAVAILABLE"
  | "CANCELLED";
export type QQMusicControlSubmissionState =
  | "accepted"
  | "not_submitted"
  | "rejected"
  | "unknown";

export type QQMusicPlayerStateObservation =
  | "changed"
  | "unchanged"
  | "failed"
  | "not_observed";

export interface QQMusicObservedState {
  readonly available: boolean;
  readonly playbackState: PlaybackState;
  readonly track?: MusicTrack;
}

function resolvePlayerStateObservation(
  action: QQMusicControlAction,
  before: DesktopBridgeSnapshot,
  after: DesktopBridgeSnapshot,
): QQMusicPlayerStateObservation {
  if (!after.available) return "failed";
  if (!before.available) return "not_observed";

  if (action === "next" || action === "previous") {
    if (before.track === undefined || after.track === undefined) return "not_observed";
    return before.track.id === after.track.id ? "unchanged" : "changed";
  }

  const playbackChanged =
    before.playbackState.paused !== after.playbackState.paused ||
    before.playbackState.loaded !== after.playbackState.loaded;
  return playbackChanged ? "changed" : "unchanged";
}
...
export type QQMusicControlResult =
  | {
      ...
      /** The GSMTC control API accepted the command. */
      readonly commandSubmission: "accepted";
      /** A single bounded post-command read; this is not a renderer ACK. */
      readonly playerStateObservation: QQMusicPlayerStateObservation;
      /** The state returned by that read, when the QQMusic session remains available. */
      readonly observedState?: QQMusicObservedState;
    }
  | {
      readonly ok: false;
      readonly action: QQMusicControlAction;
      ...
      readonly commandSubmission: Exclude<QQMusicControlSubmissionState, "accepted">;
       readonly playerStateObservation: QQMusicPlayerStateObservation;
    };
...
export class MusicService {
  ...
  async controlQQMusic(
    ...
    const before = this.desktopBridge.getSnapshot();
    const failure = (
      error: QQMusicControlFailureCode,
      message: string,
      commandSubmission: Exclude<QQMusicControlSubmissionState, "accepted">,
    ): QQMusicControlResult => ({
      ok: false,
      action,
      target: "QQMusic",
      error,
    ...
    if (result.ok) {
      // next/previous already performs the one delayed poll inside the bridge;
      // the other transport actions need the same single post-command read.
      if (bridgeAction !== "next" && bridgeAction !== "prev") {
        await this.desktopBridge.pollAfterControl();
      }
      const after = this.desktopBridge.getSnapshot();
      const observedState: QQMusicObservedState | undefined = after.available
        ? {
            available: true,
            playbackState: after.playbackState,
            ...(after.track === undefined ? {} : { track: after.track }),
          }
        : undefined;
      return {
        ok: true,
        ...
        playerStateObservation: resolvePlayerStateObservation(action, before, after),

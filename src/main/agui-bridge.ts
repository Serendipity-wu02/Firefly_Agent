...
export interface AguiRunInput {
  ...
  workDocuments?: Array<{ name: string; path: string; requiredEndLine?: number; totalLines?: number; partialAccepted?: boolean }>;
...
export function registerAgUiIpc(
  ...
  ipc.handle(IPC.AGUI_RUN, async (event: IpcMainInvokeEvent, rawInput: unknown) => {
    ...
    if (requiredWorkReads.some((scope) => !isWorkReadScopeCurrent(scope))) {
      ...
      lifecycle?.onConversationEnded();
      throw new Error("WORK_READ_SCOPE_CHANGED:所选文件已变化，请重新发送并确认读取范围");
    }
    ...
    built = await perf.track("build_options", () => buildOptionsFn!({
      ...
      workDocuments: workDocuments.map((attachment) => ({
        ...
        path: attachment.kind === "document" && attachment.readScope ? attachment.readScope.path : attachment.filePath,
        ...(attachment.kind === "document" && attachment.readScope ? {
          requiredEndLine: attachment.readScope.endLine,
          totalLines: attachment.readScope.totalLines,
          partialAccepted: attachment.readScope.partialAccepted,
        } : {}),
      })),
      mode,
    ...
    const sub = agent.runWithEvents(options).subscribe({
      ...
      complete: async () => {
        ...
        if (requiredWorkReads.length > 0) {
          try {
            const snapshot = await getConversationTranscriptStore(app.getPath("userData")).read(sessionId);
            const report = evaluateWorkFileEvidence({
              runId,
              scopes: requiredWorkReads,

...
describe("pet window move controller", () => {
  ...
  it("applies each renderer-coalesced position without another timer", () => {
    ...
    expect(window.setPosition).toHaveBeenCalledTimes(3);
    expect(window.setPosition).toHaveBeenCalledWith(130, 230, false);
  });

  it("persists the final applied position when dragging ends", () => {
    const window = createWindow();
    const persist = vi.fn();

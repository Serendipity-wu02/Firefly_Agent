...
export function registerChatsIpc(
  ...
  ipc.handle(IPC.CHATS_EXPORT_WORK_MARKDOWN, async (event, id: unknown) => {
    ...
    const markdown = session && createWorkMarkdownSnapshot(session);
    if (!markdown) return { ok: false, error: "work-task-not-finished" };
    const window = BrowserWindow.fromWebContents(event.sender);
    if (!window) return { ok: false, error: "no-window" };
    const result = await dialog.showSaveDialog(window, {
      title: "导出 Work 任务",
      defaultPath: "work-task.md",
  ...
  ipc.handle(
    ...
    async (event, payload: { sessionId?: unknown; entry?: unknown }) => {
      ...
      const requested = session.mode === "work" && entry.requireDocumentRead === true;
      const documents = (entry.attachments ?? []).filter((attachment) => attachment.kind === "document");
      if (requested && documents.length === 0) return { ok: false, error: "no-document-selected" };
      const attachments = [] as NonNullable<chatsStore.PendingChatMessageInput["attachments"]>;
      for (const attachment of entry.attachments ?? []) {
        const { readScope: _ignored, ...stableAttachment } = attachment;
        if (!requested || attachment.kind !== "document") {
          attachments.push(stableAttachment);
          continue;
        }
        let scope;
        try {
          scope = inspectWorkReadFile(attachment.filePath);
        } catch (error) {
          return { ok: false, error: error instanceof Error ? error.message : "file-not-readable" };
        }
        if (scope.totalLines > WORK_READ_PAGE_LINES) {
          const window = BrowserWindow.fromWebContents(event.sender);
          if (!window) return { ok: false, error: "no-window" };
          const choice = await dialog.showMessageBox(window, {
            type: "question",
            title: "确认读取范围",
            message: `${scope.name} 共 ${scope.totalLines} 行。read_file 每次最多读取 ${WORK_READ_PAGE_LINES} 行。`,
            detail: `完整读取需要逐段读取 1–${scope.totalLines} 行。也可只要求读取 1–${WORK_READ_PAGE_LINES} 行；${WORK_READ_PAGE_LINES + 1}–${scope.totalLines} 行将保持未覆盖。`,
            buttons: ["完整读取", `只读前 ${WORK_READ_PAGE_LINES} 行`, "取消"],
            defaultId: 0,
            cancelId: 2,
          });
          if (choice.response !== 0 && choice.response !== 1) return { ok: false, canceled: true };
          if (choice.response === 1) scope = { ...scope, endLine: WORK_READ_PAGE_LINES, partialAccepted: true };
        }
        attachments.push({ ...stableAttachment, readScope: scope });
      }
      if (attachments.some((attachment) => attachment.readScope && !isWorkReadScopeCurrent(attachment.readScope))) {
        return { ok: false, error: "file-changed-before-enqueue" };
      }
      const result = chatsStore.enqueuePendingMessage(sessionId, { ...entry, attachments, requireDocumentRead: requested });

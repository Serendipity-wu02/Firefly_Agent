...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "firefly-log-sink-"));
...
describe("createFileLogSink", () => {
  it("appends a timestamped line to the file", () => {
    const logPath = path.join(tmpDir, "firefly.log");
    const sink = createFileLogSink(logPath);
    sink(makeEntry());
    const content = fs.readFileSync(logPath, "utf8");
    expect(content).toContain("2026-09-01 09:32:24.123");
    expect(content).toContain("INFO  Firefly         hello");
  });

  it("appends multiple entries in order", () => {
    const logPath = path.join(tmpDir, "firefly.log");
    const sink = createFileLogSink(logPath);
    sink(makeEntry({ message: "first", line: "INFO  Firefly         first" }));
    sink(makeEntry({ message: "second", line: "INFO  Firefly         second" }));
    const content = fs.readFileSync(logPath, "utf8");
  ...
  it("survives append failures without throwing", () => {
    ...
    const logPath = path.join(tmpDir, "no-such-dir", "firefly.log");
    const sink = createFileLogSink(logPath);
  ...
  it("rotates automatically on write when the file exceeds maxBytes", () => {
    ...
    sink(makeEntry({ message: "big-entry", line: "INFO  Firefly         big-entry" }));
    sink(makeEntry({ message: "next", line: "INFO  Firefly         next" }));
...
describe("installFileLogSink", () => {
  it("writes to <userData>/logs/firefly.log and returns an uninstall fn", () => {
    const uninstall = installFileLogSink(tmpDir);
    try {
      setLogLevel("info");
      logger.info("Firefly", "sink-e2e");
      const logPath = path.join(tmpDir, "logs", "firefly.log");
      ...
      logger.info("Firefly", "after-uninstall");
  ...
  it("rotates through the real logger with a small maxBytes", () => {
    ...
    try {
      ...
      for (let i = 0; i < 5; i++) {
        logger.info("Firefly", `rotation-${i}-${"x".repeat(60)}`);
      }
      ...
      logger.info("Firefly", "after-rotate");

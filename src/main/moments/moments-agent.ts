...
export interface MomentsAgentDeps {
  ...
  /** 提交流萤动态（store 串行队列内含开关复核）；返回 applied 表示真的落库 */
...
export interface MomentsAgent {
  /** 流萤对动态表态的决策：动态已删 → stale；模型失败 → retry；输出非法 → invalid。
   *  mentioned：用户在这条动态里 @ 了流萤——prompt 会感知点名，延迟也走秒回档。 */
  decideUserPostReaction: (postId: string, mentioned?: boolean) => Promise<ReactionDecideOutcome>;
  /** 流萤被回复后的决策：post 已删 / 触发评论已删 → stale */

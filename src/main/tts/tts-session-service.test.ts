...
describe("TtsSessionService", () => {
  ...
  it("returns cancelled when the aborted engine rejects", async () => {
    let reject!: (error: Error) => void;
    const service = new TtsSessionService(() => new Promise((_resolve, fail) => { reject = fail; }));
    const pending = service.start(request("r-aborted"));
    expect(service.cancel("r-aborted")).toBe(true);
    reject(new Error("GPT-SoVITS 合成已取消"));
    await expect(pending).resolves.toEqual({ requestId: "r-aborted", status: "cancelled" });
  });

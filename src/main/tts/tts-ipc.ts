...
import { app, ipcMain, BrowserWindow, type IpcMainEvent } from "electron";
...
export interface TtsIpcRegistration {
  readonly sessionService: TtsSessionService;
  readonly getSettings: () => TtsSettings;
  readonly dispose: () => void;
}

const VALID_TTS_ENGINES: ReadonlySet<string> = new Set(["off", "gptsovits"]);

export function registerTtsIpc(options: TtsIpcOptions): TtsIpcRegistration {
  ...
  const webContentsCreated = (_event: Electron.Event, contents: Electron.WebContents): void => {
  ...
  const speakingChanged = (
    event: IpcMainEvent,
    payload: { isSpeaking?: unknown; requestId?: unknown },
  ): void => {
  ...
  const dispose = (): void => {
    if (disposed) return;
    disposed = true;
    sessionService.cancelAll();
    app.removeListener("web-contents-created", webContentsCreated);
    ipcMain.removeListener(IPC.PET_SPEAKING_CHANGED, speakingChanged);
  ...
  try {
    ...
    registeredChannels.add(IPC.TTS_GET_SETTINGS);

    ipcMain.handle(IPC.TTS_SAVE_SETTINGS, async (_event, settings: TtsSettings) => {
      saveSettings(settings);
      return true;
    });
    registeredChannels.add(IPC.TTS_SAVE_SETTINGS);
    ...
    registeredChannels.add(IPC.TTS_ACQUIRE_PLAYBACK);
    ...
    registeredChannels.add(IPC.TTS_RELEASE_PLAYBACK);

    ipcMain.on(IPC.PET_SPEAKING_CHANGED, speakingChanged);
  } catch (error: unknown) {
    dispose();
    throw error;
  }

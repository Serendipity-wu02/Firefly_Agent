/**
 * @file relationships.test.ts
 * @description Current relationship and companion runtime contract.
...
import type { CharacterRelationship } from "../../../dist/main/main/character/relationship-types.js";

type LegacyRelationshipProbe = CharacterRelationship & Partial<Record<"affection" | "level" | "score" | "unlockedLevel", unknown>>;
...
test("1. Core Companion Relationship: Trailblazer is deeply recognized with exact canonical facts and address", async () => {
  ...
  assert.ok(rel.canonicalFacts.some((fact: string) => fact.includes("一日导游")));
  assert.ok(rel.canonicalFacts.some((fact: string) => fact.includes("秘密基地")));
  assert.ok(rel.canonicalFacts.some((fact: string) => fact.includes("烟花") || fact.includes("热砂海选")));

  // Boundaries & anti-fabrication
  assert.equal(rel.boundaries.canFabricateSharedPast, false);
  assert.ok(rel.boundaries.forbiddenTropes.some((trope: string) => trope.includes("严禁捏造")));
});
...
test("2. Stellaron Hunters Companions: Kafka, Silver Wolf, Blade, and Elio verified with official facts", async () => {
  ...
  assert.ok(kafka.canonicalFacts.some((fact: string) => fact.includes("格拉默废墟")));
  ...
  assert.ok(sw.canonicalFacts.some((fact: string) => fact.includes("游戏卡带") || fact.includes("入梦池")));

  // Blade
  const blade = RelationshipRegistry.findRelationship("刃");
  assert.ok(blade);
  assert.equal(blade.category, "stellaron_hunter");
  assert.ok(blade.canonicalFacts.some((fact: string) => fact.includes("不超过三句") || fact.includes("匹诺康尼")));

  // Elio
  const elio = RelationshipRegistry.findRelationship("艾利欧");
  assert.ok(elio);
  assert.equal(elio.category, "stellaron_hunter");
  assert.ok(elio.canonicalFacts.some((fact: string) => fact.includes("剧本")));
});
...
test("3. Astral Express & Key Penacony Contacts: Verified official contacts with specific attitudes", async () => {
  ...
  assert.ok(robin.canonicalFacts.some((fact: string) => fact.includes("流梦礁")));

  // Acheron
  const acheron = RelationshipRegistry.findRelationship("黄泉");
  assert.ok(acheron);
  assert.equal(acheron.category, "penacony_contact");
  assert.ok(acheron.canonicalFacts.some((fact: string) => fact.includes("自灭者") || fact.includes("萨姆")));

  // Gallagher
  const gallagher = RelationshipRegistry.findRelationship("加拉赫");
  assert.ok(gallagher);
  assert.equal(gallagher.category, "penacony_contact");
  assert.ok(gallagher.canonicalFacts.some((fact: string) => fact.includes("猎犬家系") || fact.includes("帮过流萤")));

  // Sparkle
  const sparkle = RelationshipRegistry.findRelationship("花火");
  assert.ok(sparkle);
  assert.equal(sparkle.category, "penacony_contact");
  assert.ok(sparkle.canonicalFacts.some((fact: string) => fact.includes("黄金时刻") || fact.includes("烟花") || fact.includes("假面愚者")));
});
...
test("7. Zero Gamification / Zero Numeric Affection: Relationship model has zero levels, points, or grinding stats", async () => {
  const { RelationshipRegistry } = await import(`file://${relationshipRegistryPath}`);
  const allRelationships = Array.from(RelationshipRegistry.getRelationships().values()) as LegacyRelationshipProbe[];

/**
 * @file persona-policy.test.ts
 * @description Current persona and character policy contract.
 */
...
test("6. Persona System Prompt Projection: Generates high-fidelity, complete System Prompt", async () => {
  ...
  assert.ok(prompt.includes("【当前内在状态】"));
  assert.ok(prompt.includes("关切担忧"));
  assert.ok(!prompt.includes("状态基线（兼容）"));
...
test("8. State / Persona Boundary: State changes do not mutate immutable Persona values", async () => {
  ...
  // Prompt state is derived from semantic request context, not numeric stats.
  const exhaustedPrompt = SystemPromptBuilder.build({
    userPrompt: "我现在身体很难受……",
  });

  assert.ok(exhaustedPrompt.includes("【当前内在状态】"));
  assert.ok(exhaustedPrompt.includes("关切担忧"));
...
test("9. Utterance Validation: Detects forbidden words and strips stage directions", async () => {
  ...
  assert.ok(forbiddenCheck.violations.some((v: string) => v.includes("主人")));
  assert.ok(forbiddenCheck.violations.some((v: string) => v.includes("yyds")));
...
test("9.1. Character owns a Persona-shaped clarification for ambiguous music control", async () => {
  const { CharacterPolicyEngine } = await import(`file://${characterPolicyPath}`);
  const engine = CharacterPolicyEngine.getInstance();
  const clarification = engine.createMusicControlClarification();

  assert.match(clarification, /开拓者/u);
  assert.match(clarification, /下一首/u);
  assert.match(clarification, /上一首/u);
  assert.match(clarification, /嗯…/u);
  assert.doesNotMatch(clarification, /已经|提交|观察到|成功/u);
});

...
test("10. Semantic Independence: Behavior and embodiment use semantic request context", async () => {
  const { BehaviorRuntime } = await import(`file://${behaviorRuntimePath}`);
  const { EmbodimentAdapter } = await import(`file://${embodimentAdapterPath}`);

  const d1 = BehaviorRuntime.decide({ userPrompt: "流萤，今天辛苦啦！", mode: "daily" });
  const d2 = BehaviorRuntime.decide({ userPrompt: "流萤，今天辛苦啦！", mode: "daily" });
...
test("16. Semantic State Independence: Behavior and embodiment are deterministic for the same request", async () => {
  const { BehaviorRuntime } = await import(`file://${behaviorRuntimePath}`);
  const { EmbodimentAdapter } = await import(`file://${embodimentAdapterPath}`);

  const decisionLow = BehaviorRuntime.decide({ userPrompt: "流萤，今天辛苦啦！", mode: "daily" });
  const decisionHigh = BehaviorRuntime.decide({ userPrompt: "流萤，今天辛苦啦！", mode: "daily" });

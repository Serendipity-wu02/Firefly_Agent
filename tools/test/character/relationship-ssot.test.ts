/**
 * @file relationship-ssot.test.ts
...
type LegacyRelationshipProbe = CharacterRelationship & Partial<Record<"affection" | "level" | "score" | "unlocked", unknown>>;
...
interface RawRelationshipFact {
  entity?: string;
  fact?: string;
}

function isRawRelationshipFact(value: unknown): value is RawRelationshipFact {
...
test("1. Production Canonical Loading: Relationships are dynamically loaded from resources/knowledge/facts.yaml", async () => {
  ...
  const rawYaml = yaml.load(fs.readFileSync(path.join(projectRoot, "src", "main", "character", "resources", "knowledge", "facts.yaml"), "utf-8")) as unknown;
  const rawFactsList: RawRelationshipFact[] = Array.isArray(rawYaml)
    ? rawYaml.filter(isRawRelationshipFact)
    : typeof rawYaml === "object" && rawYaml !== null && "facts" in rawYaml && Array.isArray(rawYaml.facts)
      ? rawYaml.facts.filter(isRawRelationshipFact)
      : typeof rawYaml === "object" && rawYaml !== null
        ? Object.values(rawYaml).flatMap((value) => Array.isArray(value) ? value.filter(isRawRelationshipFact) : [])
        : [];
  const trailblazerRawFacts = rawFactsList
    .filter((fact): fact is RawRelationshipFact & { entity: string; fact: string } =>
      fact.entity === "开拓者" && typeof fact.fact === "string")
  ...
  for (const rawFact of trailblazerRawFacts) {
    assert.ok(
      trailblazer.canonicalFacts.some((fact: string) => fact.includes(rawFact) || rawFact.includes(fact)),
...
test("2. Dynamic SSoT Synchronization & Deletion Transparency: Modifying facts changes Runtime, deleting facts leaves 0 fallback", async () => {
  ...
  try {
    ...
    assert.ok(customTb.evidenceSources.some((source: string) => source.includes("开拓者共鸣.md")));
  ...
  assert.ok(restoredTb);
    assert.ok(restoredTb.canonicalFacts.some((fact: string) => fact.includes("一日导游") || fact.includes("秘密基地")));
...
test("6. Non-Gamification Verification: Zero numerical affection, progression, or level stats", async () => {
  const { RelationshipRegistry } = await import(`file://${relationshipRegistryPath}`);
  const allRels = Array.from(RelationshipRegistry.getRelationships().values()) as LegacyRelationshipProbe[];

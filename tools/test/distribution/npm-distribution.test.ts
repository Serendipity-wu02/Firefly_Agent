...
const npmCacheDir = fs.mkdtempSync(path.join(os.tmpdir(), 'firefly-npm-cache-'));
...
test('1. package.json Distribution Fields Integrity', () => {
  ...
  assert.equal(pkg.bin.firefly, './dist/cli/firefly.mjs');
  assert.equal(pkg.bin['firefly-agent'], './dist/cli/firefly.mjs');
  ...
  assert.ok(pkg.files.includes('src/main/rag/knowledge'), 'files must include src/main/rag/knowledge');
  assert.ok(pkg.files.includes('src/main/character/resources'), 'files must include src/main/character/resources');
  assert.ok(pkg.files.includes('src/main/settings/settings.example.json'), 'files must include src/main/settings/settings.example.json');
...
test('2. CLI Entry Point Executable & Options', () => {
  const cliPath = path.join(projectRoot, 'dist', 'cli', 'firefly.mjs');
  assert.ok(fs.existsSync(cliPath), 'dist/cli/firefly.mjs must exist after build');
...
test('3. Clean Directory Tarball Installation & Resource Isolation Test', () => {
  ...
  try {
    ...
    // Step C: Extract the real npm tarball in a clean directory.
    // The package contents and CLI are validated without downloading production dependencies.
    ...
    const installedPkgDir = path.join(tempDir, 'package');
    ...
    const mustExistFiles = [
      path.join(installedPkgDir, 'dist', 'cli', 'firefly.mjs'),
      ...
      path.join(installedPkgDir, 'src', 'main', 'rag', 'knowledge', 'chunks.json'),
      path.join(installedPkgDir, 'src', 'main', 'settings', 'settings.example.json'),
      path.join(installedPkgDir, 'dist', 'main', 'main', 'music', 'scripts', 'qqmusic_gsmtc.ps1'),
    ];
    ...
    const mustNotExistFiles = [
      ...
      path.join(installedPkgDir, 'src', 'cli'),
    ...
    const installedCli = path.join(installedPkgDir, 'dist', 'cli', 'firefly.mjs');

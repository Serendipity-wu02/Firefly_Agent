/**
 * @file environment-baseline.test.ts
...
test('2. npm Executable Version Baseline', () => {
  ...
  try {
    ...
    assert.fail(`Failed to execute npm --version: ${message}`);
  }
...
test('6. Project-Level npm 11 Launcher (tools/npm/npm.mts, tools/npm/npm.cmd & tools/npm/npm.ps1)', () => {
  const npmScriptPath = path.join(projectRoot, 'tools', 'npm', 'npm.mts');
  assert.ok(fs.existsSync(npmScriptPath), 'tools/npm/npm.mts launcher must exist');
  ...
  // Verify tools/npm/npm.mts invokes npm 11 without requiring global upgrade
  const output = execSync(`"${process.execPath}" --experimental-strip-types "${npmScriptPath}" -v`, {
    encoding: 'utf-8',
    cwd: projectRoot
  }).trim();

  const major = parseInt(output.split('.')[0], 10);
  assert.equal(major, 11, `tools/npm/npm.mts must output npm 11.x, actual output: ${output}`);
});

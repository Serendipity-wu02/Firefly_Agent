/**
 * @file font-size-live-sync.test.ts
...
test("1. Canonical font-size owner returns an effective UI snapshot", () => {
  ...
  const managerSource = read("src/main/settings/settings-manager.ts");
...
test("2. A font-size selection persists through the existing main settings IPC", () => {
  ...
  const mainSource = read("src/main/application/default-dependencies.ts");
  ...
  assert.ok(mainSource.includes("dependencies.settingsManager.save(newSettings)"));
  assert.ok(mainSource.includes("dependencies.windowManager.broadcast(IPC.SETTINGS_CHANGED, updated)"));
});

/**
 * @file desktop-pet.test.ts
 * @description Current desktop pet presentation and interaction contract.
...
// Import CharacterPolicyEngine, WindowManager, and presentation components
...
test("3. Default Expression Determinism: Firefly.model3.json binds expression00 to Expressions_0_File_0.json", () => {
  const modelJsonPath = path.join(rootDir, "src", "renderer", "models", "Firefly.model3.json");
  ...
  const exp0 = modelData.FileReferences.Expressions.find((e: { Name: string }) => e.Name === "expression00");
  assert.ok(exp0, "expression00 must be defined in model3.json");
  assert.equal(exp0.File, "Expressions/Expressions_0_File_0.json");

  const exp0File = path.join(rootDir, "src", "renderer", "models", exp0.File);
...
test("5. Default Idle Motion: Idle/0 is registered and points to Motions_Tick2_0_File_0.json", () => {
  ...
  assert.ok(fs.existsSync(path.join(rootDir, "src", "renderer", "models", idle0.File)), "Motions_Tick2_0_File_0.json must exist");
});
...
test("15. Drag End Non-Interference: Ending a drag does not force idle or talking action", () => {
  const interactionSource = fs.readFileSync(path.join(rootDir, "src", "renderer", "live2d", "interaction.ts"), "utf-8");
  assert.ok(!interactionSource.includes('playActionId("idle")'));
  assert.ok(!interactionSource.includes('playActionId("talking")'));
});

test("16. Expression Reset: Temporary expressions restore the active persistent behavior expression", () => {
  const mainRendererSource = fs.readFileSync(path.join(rootDir, "src", "renderer", "main.ts"), "utf-8");
  assert.ok(mainRendererSource.includes("temporary"));
  assert.ok(mainRendererSource.includes("manager.setExpression(currentPersistentExpression)"));
  assert.ok(!mainRendererSource.includes('manager.setExpression("expression00")'));
});

...
const { ChatHistoryStore } = await import("../../../src/main/chat/chat-history.ts");
...
test("4. Settings window is singleton and reuses the canonical settings backend", () => {
  const wmSource = fs.readFileSync(path.join(rootDir, "src", "main", "windows", "window-manager.ts"), "utf-8");
  const settingsSource = fs.readFileSync(path.join(rootDir, "src", "main", "settings", "settings-manager.ts"), "utf-8");
  ...
  assert.ok(compositionSource.includes("const settingsManager = new SettingsManager(configPath)"), "Composition must instantiate one SettingsManager");
  assert.ok(compositionSource.includes("dependencies.settingsManager.save(newSettings)"), "Settings IPC must save through SettingsManager");
...
test("13. GUI Fix Round 2: Status dot, independent Mood settings sync, and Chat shell layout", () => {
  ...
  assert.ok(compositionSource.includes("dependencies.windowManager.broadcast(IPC.SETTINGS_CHANGED, updated)"), "Main must broadcast the saved settings to every window");
...
test("14. Typed maximize state: native state reaches renderer and shell/icon presentation transitions", () => {
  ...
  assert.ok(
    compositionSource.includes("ipcMain.handle(IPC.WINDOW_GET_STATE"),
    "Main must answer the typed window state query",
  );
...
test("15. Mood visibility survives Chat restore without reopening a user-hidden panel", () => {
  const wmSource = fs.readFileSync(path.join(rootDir, "src", "main", "windows", "window-manager.ts"), "utf-8");

  assert.ok(
    wmSource.includes("private summaryPreferredVisible = true;"),
    "WindowManager must own the in-memory user visibility preference for Mood",
  );
  assert.ok(
    wmSource.includes("if (this.summaryPreferredVisible && this.summaryWindow && !this.summaryWindow.isDestroyed())"),
    "Chat restore/show must not reopen a user-hidden Mood window",
  );
  assert.ok(
    wmSource.includes("this.summaryPreferredVisible = false;\n      this.summaryWindow.hide();"),
    "Hiding Mood through its toggle must retain the hidden preference",
  );
  assert.ok(
    wmSource.includes("this.summaryPreferredVisible = true;\n      this.summaryWindow.show();"),
    "Showing Mood through its toggle must restore the visible preference",
  );
});

test("16. Process-local Chat history restores visible messages without storing system/tool context", () => {
  const store = new ChatHistoryStore();
  store.replace([
    { id: "system", role: "system", content: "internal" },
    { id: "u1", role: "user", content: "你好" },
    { id: "tool", role: "tool", content: "internal tool output", toolCallId: "call-1" },
    { id: "a1", role: "assistant", content: "你好，开拓者。" },
  ]);

  assert.deepEqual(
    store.getMessages().map((message: { id: string; role: string }) => [message.id, message.role]),
    [
      ["u1", "user"],
      ["a1", "assistant"],
    ],
  ...
  snapshot[0].content = "changed outside the store";
  assert.equal(store.getMessages()[0]?.content, "你好");

  const newProcessStore = new ChatHistoryStore();
  assert.deepEqual(newProcessStore.getMessages(), [], "a new Main process store starts without the old transcript");
});

test("17. Chat IPC and renderer expose the existing history channel", () => {
  const chatIpcSource = fs.readFileSync(path.join(rootDir, "src", "main", "chat", "chat-ipc.ts"), "utf-8");
  const preloadSource = fs.readFileSync(path.join(rootDir, "src", "preload", "index.ts"), "utf-8");
  const rendererTypesSource = fs.readFileSync(path.join(rootDir, "src", "renderer", "electron.d.ts"), "utf-8");
  const appSource = fs.readFileSync(path.join(rootDir, "src", "renderer", "ui", "App.tsx"), "utf-8");

  assert.ok(chatIpcSource.includes("ipcMain.handle(IPC.CHAT_GET_HISTORY"));
  assert.ok(chatIpcSource.includes("ipcMain.removeHandler(IPC.CHAT_GET_HISTORY)"));
  assert.ok(preloadSource.includes("getHistory: (): Promise<ChatMessage[]>"));
  assert.ok(rendererTypesSource.includes("getHistory: () => Promise<ChatMessage[]>"));
  assert.ok(appSource.includes("window.chat.getHistory()"));
  assert.ok(appSource.includes("setMessages(nextMessages)"));
  assert.ok(chatIpcSource.includes("seedSubmittedHistoryIfEmpty"));
  assert.ok(chatIpcSource.includes("const effectiveHistory = historyStore.getMessages()"));
  assert.ok(chatIpcSource.includes("history: effectiveHistory"));
  assert.ok(chatIpcSource.includes("history: historyStore.getMessages()"));
  assert.ok(!chatIpcSource.includes("history: result.transcript"), "internal Agent transcript must not enter visible Chat history");
  assert.ok(appSource.includes("useState<ChatMessage[]>([])"), "Chat must wait for Main history before adding the initial greeting");
  assert.ok(appSource.includes("chatHistoryLoadPromiseRef"), "restore and send must share one history request");
  assert.ok(appSource.includes("const nextMessages = history.length > 0 ? history : createInitialChatMessages()"));
  assert.ok(appSource.includes("setMessages(res.history)"), "renderer must adopt Main's authoritative response history");
  assert.ok(!appSource.includes("setMessages((prev) => [...prev, asstMsg])"), "renderer must not append a second assistant message");
});

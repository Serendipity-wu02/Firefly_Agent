...
import type { MemoryRecord } from "../../../dist/main/main/memory/memory-types.js";
...
test("14. Multiple Scopes: Multi-scope array filter matches records from any specified scope", async () => {
  ...
  await store.saveRecord(createMockRecord("m_conv", "对话上下文", "B", { scope: "session" }));
  await store.saveRecord(createMockRecord("m_global", "系统设定", "C", { scope: "global" }));

  const retriever = new MemoryRetriever(store);
  const multiScope = await retriever.retrieve({ scope: ["user", "session"] });

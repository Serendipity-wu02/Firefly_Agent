...
import type {
  ...
  PreferenceItem,
  RelationEdge,
} from "../../../dist/main/main/memory/memory-types.js";

function memoryRecord(overrides: Partial<MemoryRecord> & Pick<MemoryRecord, "id">): MemoryRecord {
  return {
    layer: "L1_EPISODIC",
    category: "event",
    scope: "user",
...
function preferenceItem(overrides: Partial<PreferenceItem> & Pick<PreferenceItem, "id">): PreferenceItem {
  return {
    category: "custom",
...
function relationEdge(overrides: Partial<RelationEdge> & Pick<RelationEdge, "id">): RelationEdge {
  return {
    subject: "test-subject",
...
test("2. Pinned / L2 Decay Immunity: Pinned and L2 semantic records are 100% immune to decay", () => {
  const decayEngine = new MemoryDecayEngine({ halfLifeDays: 7 });
  const now = 1000000000000;
  const pinnedRecord = memoryRecord({
    id: "mem_pinned",
    layer: "L1_EPISODIC",
    category: "event",
    scope: "user",
...
test("3. Expiration vs Decay: Explicit TTL expiration triggers pruning cleanly", () => {
  ...
  const now = 1000000000000;
  const ephemeralRecord = memoryRecord({
    id: "mem_temp",
    layer: "L0_WORKING",
...
test("4. Decay Read-Only Safety: Retrieval operations NEVER mutate store", async () => {
  ...
  const retriever = new MemoryRetriever(store, DEFAULT_RETRIEVAL_POLICY);

  const now = 1000000000000;
  await store.saveRecord(memoryRecord({
    id: "mem_read_only",
    layer: "L1_EPISODIC",
    category: "preference",
    scope: "user",
...
test("5. Explicit Correction Resolution: Explicit correction supersedes older statements", () => {
  ...
  const newCorrection = memoryRecord({
    id: "mem_new",
    layer: "L2_SEMANTIC",
    category: "profile",
    scope: "user",
...
test("10. Superseded Memory: Superseded records are tagged in return structure", () => {
  const resolver = new MemoryConflictResolver();
  const r1 = memoryRecord({ id: "1", layer: "L1_EPISODIC", category: "event", scope: "user", key: "k", value: "v1", updatedAt: 10, createdAt: 10, accessCount: 0, lastAccessedAt: 10, importance: 0.5, confidence: 1 });
  const r2 = memoryRecord({ id: "2", layer: "L1_EPISODIC", category: "event", scope: "user", key: "k", value: "v2", updatedAt: 20, createdAt: 20, accessCount: 0, lastAccessedAt: 20, importance: 0.5, confidence: 1 });
...
test("11. Consolidation & Promotion: L1 promoted to L2 when access frequency >= 3", () => {
  ...
  assert.ok(evalResult.promotedRecord);
...
test("12. Duplicate Consolidation: Consolidates multiple related records into unified record", () => {
  ...
  const records: MemoryRecord[] = [
    memoryRecord({ id: "1", layer: "L1_EPISODIC", category: "preference", scope: "user", key: "爵士乐", value: "常听爵士乐", importance: 0.7, accessCount: 2, entities: ["爵士"], updatedAt: 100, createdAt: 100, lastAccessedAt: 100 }),
    memoryRecord({ id: "2", layer: "L1_EPISODIC", category: "preference", scope: "user", key: "爵士乐", value: "喜欢轻爵士与萨克斯", importance: 0.85, accessCount: 3, entities: ["萨克斯"], updatedAt: 200, createdAt: 200, lastAccessedAt: 200 }),
  ];
...
test("16. Malformed Memory Safety: Degraded records with missing fields are handled safely", () => {
  const decayEngine = new MemoryDecayEngine();
  const malformed = { id: "broken" } as unknown as MemoryRecord;
...
test("17. Deterministic Conflict Resolution: Strict order yields identical resolution regardless of input permutation", () => {
  const resolver = new MemoryConflictResolver();
  const r1 = memoryRecord({ id: "1", layer: "L1_EPISODIC", category: "profile", scope: "user", key: "k", value: "v1", updatedAt: 100, createdAt: 100, accessCount: 0, lastAccessedAt: 100, importance: 0.5, confidence: 1 });
  const r2 = memoryRecord({ id: "2", layer: "L1_EPISODIC", category: "profile", scope: "user", key: "k", value: "v2", updatedAt: 200, createdAt: 200, accessCount: 0, lastAccessedAt: 200, importance: 0.5, confidence: 1 });

...
import type { MemoryWriteProposal } from "../../../dist/main/main/memory/write-types.js";
...
test("5. Casual Chat Rejection: Routine greetings and chatter are blocked from store", async () => {
  ...
  const greetings: MemoryWriteProposal[] = [
...
test("6. Ephemeral State: Temporary state allocated to L0_WORKING with TTL and session scope", async () => {
  ...
  assert.ok(decision.normalizedRecord);
  assert.ok(decision.normalizedRecord.expiresAt);
...
test("11. Explicit User Correction: Supersedes previous value and records audit history in metadata", async () => {
  ...
  assert.ok(record);
  assert.equal(record.value, "流梦礁");
  assert.ok(record.metadata?.history);
  assert.equal(record.metadata.history.length, 1);
  assert.equal(record.metadata.history[0]?.value, "黄金的时刻");
  assert.equal(record.metadata.history[0]?.reason, "user_correction");
});

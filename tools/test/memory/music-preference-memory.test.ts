...
} from "../../../dist/main/main/music/music-preference-service.js";
import {
  registerMusicPreferenceSignalAdapter,
} from "../../../dist/main/main/music/music-preference-signals.js";
import {
  createMusicArtistSubject,
  createMusicTrackSubject,
} from "../../../dist/main/main/music/music-preference-subject.js";
...
} from "../../../dist/main/shared/music-preference-types.js";
import type { MemoryItem } from "../../../dist/main/shared/memory-types.js";

const testFileDir = path.dirname(fileURLToPath(import.meta.url));
const repoRoot = path.resolve(testFileDir, "../../..");

function withMemory<T>(run: (memory: FireflyMemoryService, memoryPath: string) => T): T {
...
function artistSubject(artist = "流萤"): MusicPreferenceSubject {
  const subject = createMusicArtistSubject(artist);
  assert.ok(subject);
  return subject;
}

function observed(id: string, subject: MusicPreferenceSubject, occurredAt: number): MusicPreferenceSignal {
  return {
    id,
...
function inferred(
  ...
  polarity: Exclude<MusicPreferencePolarity, "NEUTRAL">,
  occurredAt: number,
  confidence = 0.8,
): MusicPreferenceSignal {
  return {
...
test("I/J/M/V. independent inference reinforces, stays bounded, and negative evidence reduces it", () => withMemory((memory) => {
  ...
  assert.ok(two.assessment);
  assert.ok(three.assessment);
  assert.ok(three.consolidation);
...
test("context adapter consumes only available/track-change facts; controls and playback are not evidence", () => withMemory((memory) => {
  ...
  type ContextEventBase = Omit<Extract<MusicContextEvent, { type: "music.context.available" }>, "type">;
  const base: ContextEventBase = {
    source: "QQMUSIC_DESKTOP",
    playerId: "QQMusic.exe",
    observedAt: 100,
    timestamp: 100,
  ...
  eventBus.emit({ type: "music.context.available", ...base } satisfies MusicContextEvent);
  const artist = artistSubject();
  ...
  } satisfies MusicContextEvent);
  assert.equal(service.getAssessment(artist.key)?.observationCount, 1);

  const trackChanges: Array<readonly [number, string]> = [[300, "第二首"], [400, "第三首"]];
  for (const [index, title] of trackChanges) {
    eventBus.emit({
...
test("Z/AA-AJ. ownership stays in Main Memory with no direct RAG, tool, UI, or SubAgent dependency", () => {
  const serviceSource = fs.readFileSync(
    path.join(repoRoot, "src/main/music/music-preference-service.ts"),
    "utf8",
  );
  const adapterSource = fs.readFileSync(
    path.join(repoRoot, "src/main/music/music-preference-signals.ts"),
    "utf8",
  );
  const compositionSource = fs.readFileSync(
    path.join(repoRoot, "src/main/application/default-dependencies.ts"),
    "utf8",
  );
  const memorySource = fs.readFileSync(
    path.join(repoRoot, "src/main/memory/memory-service.ts"),
    "utf8",
  );
  ...
  assert.match(compositionSource, /new MusicPreferenceService\(\{ memory: memoryService \}\)/);

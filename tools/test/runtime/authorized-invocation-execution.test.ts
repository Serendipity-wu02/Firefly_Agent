...
import { createApprovalRequestId, type ApprovalRequestId, type ApprovalRequirement } from "../../../dist/main/shared/approval-types.js";
import {
...
import type {
  AuthorizedInvocationExecutionRequest,
  AuthorizedInvocationRuntimeContext,
} from "../../../dist/main/main/runtime/authorization/authorized-invocation-bridge.js";
import type { ToolDefinition, ToolContext } from "../../../dist/main/shared/tool-types.js";
...
const capabilityId: CapabilityId = createCapabilityId("execution.seam");
const toolId = "execution_seam_tool";
const requester: CapabilityRequester = { type: "main-agent", id: "firefly-main" };
const scope: SandboxScope = { kind: "filesystem", path: "C:\\workspace", access: "read" };

function createContext(
  overrides: Partial<AuthorizedInvocationRuntimeContext> = {},
): AuthorizedInvocationRuntimeContext {
  return {
...
interface InvocationOverrides {
  ...
  readonly requestCapabilityId?: CapabilityId;
  readonly bindingCapabilityId?: CapabilityId;
  readonly bindingToolId?: string;
  readonly input?: { readonly value: string };
  readonly approvalRequirement?: ApprovalRequirement;
  readonly approvalRequestId?: ApprovalRequestId;
  readonly authorizedScope?: SandboxScope;
  readonly correlation?: AuthorizedCapabilityCorrelation;
}

function createInvocation(overrides: InvocationOverrides = {}): AuthorizedCapabilityInvocation {
  ...
  const base = {
    ...
    sandboxDecision: { allowed: true as const, effectiveScope: scope },
    approvalRequirement,
...
function createFixture(
  { tool = {}, policy = {} }: { tool?: Partial<ToolDefinition>; policy?: Partial<ToolPolicyConfig> } = {},
) {
...
test("E. Raw input is rejected before execution", async () => {
  ...
  const result = await fixture.bridge.execute({
    ...
    requester,
    input: {},
  } as unknown as AuthorizedCapabilityInvocation, createContext());
...
test("H. A registered capability without an authorized binding is not executable", async () => {
  ...
  const invocation = createInvocation();
  const result = await fixture.bridge.execute(
    { request: invocation.request } as unknown as AuthorizedCapabilityInvocation,
    createContext(),
  );
...
test("I. An approval grant without a valid authorized Sandbox scope is rejected", async () => {
  ...
  const invalid = { ...invocation, effectiveScope: undefined } as unknown as AuthorizedCapabilityInvocation;
...
test("J. Approval metadata without the required provenance is rejected", async () => {
  const fixture = createFixture();
  const invocation = createInvocation();
  const invalid = { ...invocation, approvalGrant: { lifetime: "once", scope } } as unknown as AuthorizedCapabilityInvocation;
  const result = await fixture.bridge.execute(invalid, createContext());
...
test("T. A raw CapabilityRequest is not accepted as an executable invocation", async () => {
  ...
  } as unknown as AuthorizedInvocationExecutionRequest);
...
test("Y. The production composition root wires the bridge only through the Harness authorization adapter", () => {
  ...
  assert.match(compositionSource, /new AuthorizedInvocationBridge\(/);
  assert.match(compositionSource, /new HarnessAuthorizationAdapter\(/);
...
test("AB. The bridge does not widen the authorized scope", async () => {
  const fixture = createFixture();
  const narrowed: SandboxScope = { kind: "filesystem", path: "C:\\workspace\\nested", access: "read" };
  const result = await fixture.bridge.execute(
    createInvocation({
      approvalRequirement: "required",

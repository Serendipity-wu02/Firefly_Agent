...
import type { MpvController } from "../../../dist/main/main/music/mpv-controller.js";
import type { MusicTrack, PlaybackState } from "../../../dist/main/shared/music-types.js";
...
function asMpvController(player: MockMpvPlayer): MpvController {
  return player as unknown as MpvController;
}
...
test("5. MusicService -> Provider -> Player: Complete search, cache, play cycle", async () => {
  ...
  const service = new MusicService({ provider, mpv: asMpvController(mpv) });
  await service.start();
...
test("7. Music Tools: ToolDefinition execution via MusicService", async () => {
  ...
  if (!searchTool || !playTool || !statusTool || !controlTool) {
    throw new Error("Music test tools were not registered");
  }
...
test("8. FireflyAgentCore calling Music Tools: End-to-end multi-turn chat with music tool", async () => {
  const provider = new MockMusicProvider();
  const mpv = new MockMpvPlayer();
  const musicService = new MusicService({ provider, mpv: asMpvController(mpv) });
  ...
  let round = 0;
  const mockLlm: IFireflyLlmProvider = {
    id: "mock",
    name: "Mock LLM",

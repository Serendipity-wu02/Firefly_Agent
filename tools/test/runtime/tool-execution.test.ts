...
import { ToolBatchPlanner } from "../../../dist/main/main/orchestrator/tools/execution/tool-concurrency.js";
import { ToolResultPolicy } from "../../../dist/main/main/orchestrator/tools/execution/tool-result-policy.js";
...
test("1. Policy Denied: Blacklist and whitelist enforcement", async () => {
  ...
  const deniedEvents: Extract<AgentEvent, { type: "tool:denied" }>[] = [];
...
test("13. Multi-turn Tool Calling with FireflyAgentCore & ToolExecutionEngine", async () => {
  ...
  let round = 0;
  const mockProvider: IFireflyLlmProvider = {
    id: "test-provider",
    name: "Test Provider",
  ...
  const eventBus = new AgentEventBus();
  const authorizedTools: Extract<AgentEvent, { type: "tool:authorized" }>[] = [];
...
test("14. Tool retry backoff cancellation prevents a second dispatch", async () => {
  const registry = new FireflyToolRegistry();
  let executions = 0;
  registry.register({
    id: "cancelled-retry-tool",
    name: "Cancelled Retry Tool",
    description: "Returns a transient failure before the retry wait.",
    inputSchema: { type: "object", properties: {} },
    enabled: true,
    execute: async () => {
      executions++;
      return JSON.stringify({ ok: false, error: "network_timeout" });
    },
  });

  const controller = new AbortController();
  const eventBus = new AgentEventBus();
  eventBus.on("tool:retry", () => {
    setImmediate(() => controller.abort());
  });

  const engine = new ToolExecutionEngine(
    registry,
    { rules: { "cancelled-retry-tool": { maxRetries: 1, retryBackoffMs: 1_000 } } },
    eventBus,
  );
  const result = await engine.executeToolCall(
    { id: "cancelled-retry-call", name: "cancelled-retry-tool", arguments: {} },
    { runId: "tool-retry-cancel", step: 1, toolCallsCount: 1, signal: controller.signal },
  );

  assert.equal(result.isError, true);
  assert.ok(result.output.includes("tool_cancelled"));

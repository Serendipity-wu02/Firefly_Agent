...
import { ToolPolicyEvaluator } from "../../../dist/main/main/orchestrator/tools/execution/tool-policy.js";
...
import { createSandboxProfileId, type SandboxScope } from "../../../dist/main/shared/sandbox-types.js";
...
import type {
  AuthorizedInvocationRuntimeContext,
} from "../../../dist/main/main/runtime/authorization/authorized-invocation-bridge.js";
import type { UpstreamAuthorizationContext } from "../../../dist/main/shared/runtime-integration-types.js";
import type { MusicService } from "../../../dist/main/main/music/music-service.js";
...
const scope: Extract<SandboxScope, { kind: "desktop" }> = { kind: "desktop", target: "QQMusic" };
const requester: CapabilityRequester = { type: "main-agent", id: "firefly-harness" };

function pendingRecord(approvalService: ApprovalService): Promise<ApprovalRecord> {
...
interface FixtureOptions {
  ...
  readonly pilotExecute?: ToolDefinition["execute"];
...
function createFixture(options: FixtureOptions = {}) {
  ...
  engine.executeToolCall = async (call: ToolCall, context: ToolExecutionContext) => {
    state.engineCalls++;
...
function call(id: string, name = pilotToolId): ToolCall {
  return { id, name, arguments: {} };
}

type TestToolContext = ToolExecutionContext & { readonly toolCallId?: string };
...
test("C-E. Cancellation boundaries settle once without false success or duplicate execution", async () => {
  ...
  let releaseTool: () => void = () => {
    throw new Error("tool gate was not initialized");
  };
  let toolStartedResolve: () => void = () => {
    throw new Error("tool start signal was not initialized");
  };
  ...
  const running = createFixture({
    pilotExecute: async (_args: Record<string, unknown>, toolContext?: ToolContext) => {
...
test("L-M, P. Approval denial, cancellation, expiration, and ToolPolicy remain distinct", async () => {
  ...
  const upstream: UpstreamAuthorizationContext = {
    capabilityId: createCapabilityId("policy.read"),
    requestId: createCapabilityRequestId("policy-request"),
    requester,
    toolId: "policy_tool",
...
test("Q-R. music_status reads a snapshot only and ownership remains canonical", async () => {
  ...
  const statusTool = createMusicTools(musicService as unknown as MusicService).find((tool) => tool.id === pilotToolId);
  ...
  const musicToolsSource = fs.readFileSync(
    path.join(projectRoot, "src", "main", "orchestrator", "tools", "adapters", "music-tools.ts"),
    "utf8",
  );
  ...
  const subagentRoot = path.join(projectRoot, "src", "main", "orchestrator", "subagents");

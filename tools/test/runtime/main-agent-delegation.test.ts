...
} from "../../../dist/main/main/orchestrator/subagents/main-agent-delegation.js";
import { SubAgentRegistry } from "../../../dist/main/main/orchestrator/subagents/subagent-registry.js";
import { SubAgentTaskService } from "../../../dist/main/main/orchestrator/subagents/subagent-task-service.js";
import {
  DEFAULT_MUSIC_STATUS_SUBAGENT_DESCRIPTOR,
  SubAgentWorkerRuntime,
} from "../../../dist/main/main/orchestrator/subagents/subagent-worker-runtime.js";
...
import type { AgentEvent, AgentRunInput } from "../../../dist/main/shared/agent-types.js";
import type { ChatCompletionRequest } from "../../../dist/main/shared/chat-types.js";
...
import type { ToolCall, ToolContext } from "../../../dist/main/shared/tool-types.js";
import type { ToolExecutionContext } from "../../../dist/main/main/orchestrator/tools/execution/tool-execution-context.js";
import type { MainExecutionProfile } from "../../../dist/main/shared/subagent-types.js";
import type { CapabilityJsonValue } from "../../../dist/main/shared/capability-types.js";
import type { SubAgentTaskRecord } from "../../../dist/main/shared/subagent-types.js";
...
const musicScope = { kind: "desktop", target: "QQMusic" } as const;
...
function delegationArgs(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
...
function mainProfile(): MainExecutionProfile {
  return { kind: "MAIN", allowSubAgentDelegation: true };
}

function requestAt(state: IntegratedFixtureState, index: number): ChatCompletionRequest {
  const request = state.requests[index];
  assert.ok(request);
  return request;
}

function lastMessage(request: ChatCompletionRequest) {
  const message = request.messages.at(-1);
  assert.ok(message);
  return message;
}

function taskAt(fixture: { taskService: SubAgentTaskService }, index = 0): SubAgentTaskRecord {
  const task = fixture.taskService.list()[index];
  assert.ok(task);
  return task;
}

function inputSource(input: CapabilityJsonValue): string {
  assert.ok(typeof input === "object" && input !== null && !Array.isArray(input));
  const record = input as { readonly [key: string]: CapabilityJsonValue };
  const source = record.source;
  if (typeof source !== "string") throw new TypeError("Delegation test input source must be a string.");
  return source;
}

interface IntegratedFixtureOptions {
  provider?: IFireflyLlmProvider;
  maxRounds?: number;
  totalTimeoutMs?: number;
  permissionProfile?: PermissionProfile;
  statusExecute?: (args: Record<string, unknown>, context?: ToolContext) => Promise<string>;
}

interface IntegratedFixtureState {
  requests: ChatCompletionRequest[];
  events: AgentEvent[];
  statusExecutions: number;
  controlExecutions: number;
  engineCalls: Array<{ call: ToolCall; context: ToolExecutionContext }>;
  workerRuntimeCalls: number;
  statusContext?: ToolContext;
}
...
function createIntegratedFixture({
  ...
  statusExecute,
}: IntegratedFixtureOptions = {}) {
  let taskSequence = 0;
  const state: IntegratedFixtureState = {
    requests: [],
    events: [],
    statusExecutions: 0,
    controlExecutions: 0,
  ...
  let harness: FireflyHarness;
  const runtime = new SubAgentWorkerRuntime({
    registry: subAgentRegistry,
    taskService,
    bindingResolver,
    agentCore: { run: (input: AgentRunInput) => harness.run(input) },
    eventBus,
  });
  ...
  const tracedProvider: IFireflyLlmProvider = {
    ...actualProvider,
...
test("A-B. ordinary MAIN is unchanged and only an explicitly enabled MAIN sees delegation", async () => {
  ...
  assert.equal(
    requestAt(fixture.state, 0).tools?.some((entry) => entry.function.name === MAIN_AGENT_DELEGATION_TOOL_ID),
    false,
  );
  assert.equal(
    requestAt(fixture.state, 1).tools?.some((entry) => entry.function.name === MAIN_AGENT_DELEGATION_TOOL_ID),
    true,
  );
...
test("C-M. MAIN delegates once, WORKER uses the same Harness, and the same MAIN performs final synthesis", async () => {
  ...
  const firstEngineCall = fixture.state.engineCalls[0];
  assert.ok(firstEngineCall);
  assert.equal(firstEngineCall.call.name, statusToolId);
  assert.ok(firstEngineCall.context.upstreamAuthorization);
  assert.equal(firstEngineCall.context.upstreamAuthorization.requester.type, "subagent");
  ...
  assert.deepEqual(
    requestAt(fixture.state, 1).tools?.map((entry) => entry.function.name),
    [statusToolId],
  );
  assert.equal(requestAt(fixture.state, 1).messages[0]?.content.includes("流萤"), false);
  assert.equal(requestAt(fixture.state, 0).messages[0]?.content.includes("流萤"), true);
  assert.equal(requestAt(fixture.state, 3).messages[0]?.content.includes("流萤"), true);
...
test("budget is parent-bounded and preserves one MAIN continuation step", async () => {
  ...
  const provider = createProvider(async (request) => {
    ...
    const observation = JSON.parse(lastMessage(request).content);
...
test("X and AE. worker failure becomes a typed observation and the terminal task stays immutable", async () => {
  ...
  assert.throws(
    () => {
      assert.ok(execution.taskId);
      fixture.taskService.cancel(execution.taskId);
    },
    (error) => error instanceof Error && "code" in error && error.code === "TASK_ALREADY_TERMINAL",
  );
});
...
test("AA and Y. parent cancellation during Worker provider execution propagates and never resumes MAIN", async () => {
  ...
  const cancelledTask = fixture.taskService.get(task.task.taskId);
...
test("AB-AC. parent cancellation resolves the shared approval wait once and executes no tool", async () => {
  ...
  const result = await run;
  assert.equal(result.status, "cancelled");
  const cancelledApproval = fixture.approvalService.get(approval.request.approvalRequestId);
  assert.ok(cancelledApproval);
  assert.equal(cancelledApproval.state, "cancelled");
  assert.equal(taskAt(fixture).state, "cancelled");
  assert.equal(fixture.state.statusExecutions, 0);
  assert.throws(
    () => fixture.approvalService.approve(approval.request.approvalRequestId),
    (error) => error instanceof Error && "code" in error && error.code === "APPROVAL_ALREADY_RESOLVED",
  );
});
...
test("parent cancellation during delegated tool execution reaches the canonical engine signal", async () => {
  ...
  assert.equal(fixture.state.statusExecutions, 1);
  assert.equal(fixture.state.engineCalls.length, 1);
  const cancelledCall = fixture.state.engineCalls[0];
  assert.ok(cancelledCall);
  assert.ok(cancelledCall.context.signal);
  assert.equal(cancelledCall.context.signal.aborted, true);
});
...
test("AD. worker completion racing parent cancellation cannot resume the cancelled parent", async () => {
  ...
  assert.equal(taskAt(fixture).state, "succeeded");
});
...
test("I and R-U. concurrent parent runs receive only their own correlated observation", async () => {
  ...
  fixture.runtime.execute = async (taskId) => {
    ...
    const source = inputSource(running.task.input);
    if (source === "parent-a") await delay(15);
    return fixture.taskService.succeed(taskId, {
      ok: true,
      output: { source },
    });
...
test("N-Q. local mode boundary enables Chat only and does not duplicate the integration", () => {
  ...
  const proactiveSource = fs.readFileSync(
    path.join(projectRoot, "src", "main", "proactive", "proactive-scheduler.ts"),
    "utf8",
  );
...
test("AF-BO. delegation preserves the single-owner architecture and Worker isolation", () => {
  ...
  const delegationSource = fs.readFileSync(
    path.join(projectRoot, "src", "main", "orchestrator", "subagents", "main-agent-delegation.ts"),
    "utf8",
  );
  ...
  assert.equal((compositionSource.match(/new SubAgentWorkerRuntime\s*\(/g) ?? []).length, 1);
  assert.equal((compositionSource.match(/new SubAgentTaskService\s*\(/g) ?? []).length, 1);

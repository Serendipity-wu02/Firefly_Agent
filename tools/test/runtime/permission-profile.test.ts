...
import type { ApprovalRequirement } from "../../../dist/main/shared/approval-types.js";
import type { ToolSideEffect } from "../../../dist/main/shared/tool-types.js";
import type { SandboxScope } from "../../../dist/main/shared/sandbox-types.js";
import type { CapabilityAuthorizationInput } from "../../../dist/main/shared/runtime-integration-types.js";
...
const rootScope: SandboxScope = { kind: "filesystem", path: "C:\\workspace", access: "read" };

interface PipelineFixtureOptions {
  registerBinding?: boolean;
  sideEffect?: ToolSideEffect;
  declaredRequirement?: ApprovalRequirement;
  profile?: PermissionProfile;
}

function createPipelineFixture({
  registerBinding = true,
  sideEffect = "read_only" as ToolSideEffect,
  declaredRequirement = "none" as ApprovalRequirement,
  profile = "RESTRICTED_SCOPE" as PermissionProfile,
}: PipelineFixtureOptions = {}) {
  let activeProfile: PermissionProfile = profile;
...
function createAuthorizationInput(
  overrides: Partial<CapabilityAuthorizationInput<{ path: string }>> = {},
): CapabilityAuthorizationInput<{ path: string }> {
  return {
    request: {
...
test("A-C. The default and persisted schemes use the canonical four-scheme contract", () => {
  ...
  try {
    ...
    assert.equal(settings.save({ permissionProfile: "STANDARD" } as never), false);
...
test("I-K. Unknown capabilities, invalid scope, and missing bindings remain hard rejects", () => {
  ...
  const resolver = new PermissionProfilePolicyResolver();
  const profiles: PermissionProfile[] = ["READ_ONLY", "RESTRICTED_SCOPE", "ASK_EVERY_TIME", "FULL_ACCESS"];

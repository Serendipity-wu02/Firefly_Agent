...
function createMockGsmtcExecutor(initialState: Partial<GsmtcRawState> = {}): {
  executor: GsmtcExecutor;
  state: GsmtcRawState;
  executedActions: GsmtcAction[];
} {
  const state: GsmtcRawState = {
    ok: true,
    found: true,
    appId: "QQMusic.exe",
    title: "如果能成为萤火虫",
  ...
  const executedActions: GsmtcAction[] = [];

  const executor: GsmtcExecutor = async (action) => {
...
test("8. MusicTools Integration: Querying & Controlling QQ Music via Agent Tools", async () => {
  ...
  if (!statusTool || !controlTool) throw new Error("QQ Music test tools were not registered");

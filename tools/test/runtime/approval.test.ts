...
import { createSubAgentId, createSubAgentTaskId } from "../../../dist/main/shared/subagent-types.js";
...
function createInput(overrides: Partial<ApprovalRequestInput> = {}): ApprovalRequestInput {
  return {
    capabilityRequestId,
    capabilityId,
    requester: { type: "main-agent", id: "firefly-main" } satisfies CapabilityRequester,
    summary: "读取项目目录中的文件",
    reason: "完成用户请求的本地项目检查。",
    effectiveScope: effectiveScope satisfies SandboxScope,
    expiresAt: 2_000,
    ...overrides,
  };
}
...
test("C2. A process-scoped approval is a reusable in-process decision with a bounded key", () => {
  const { clock, service } = createService();
  const pending = service.createPending(createInput({ grantLifetime: "process" }));
  const resolved = service.approve(pending.request.approvalRequestId);

  assert.equal(resolved.decision?.approved, true);
  if (resolved.decision?.approved) {
    assert.equal(resolved.decision.grant.lifetime, "process");
  }

  const key = {
    capabilityId,
    toolId: "synthetic_read",
    sandboxProfileId: createSandboxProfileId("approval-process-profile"),
    scope: effectiveScope,
  } as const;
  service.rememberProcessGrant(key, pending.request.approvalRequestId, effectiveScope);
  assert.equal(service.getProcessGrant(key)?.approvalRequestId, pending.request.approvalRequestId);
  assert.equal(service.getProcessGrant(key)?.grantedAt, clock.now());

  service.revokeProcessGrants();
  assert.equal(service.getProcessGrant(key), undefined);
});
...
test("G-J. Every terminal state rejects a second resolution", () => {
  const transitions = [
    (service: ReturnType<typeof createService>["service"], id: ApprovalRequestInput["capabilityRequestId"] extends never ? never : ReturnType<typeof createApprovalRequestId>) => service.approve(id),
    (service: ReturnType<typeof createService>["service"], id: ReturnType<typeof createApprovalRequestId>) => service.deny(id),
    (service: ReturnType<typeof createService>["service"], id: ReturnType<typeof createApprovalRequestId>) => service.cancel(id),
  ];
...
test("M-P. Requester, capability, correlation, and effective scope are preserved", () => {
  ...
  const input = createInput({
    requester: {
      type: "subagent",
      id: "future-subagent",
      subAgentId: createSubAgentId("future-subagent"),
      taskId: createSubAgentTaskId("future-task"),
      parentRunId: "run-1",
    },
...
test("R. Registered capability metadata does not create an approved request", () => {
  ...
  registry.register({
    ...
    version: "1.0.0",
    category: createCapabilityCategory("synthetic"),
  });
  const { service } = createService();
  const record = service.createPending(createInput());

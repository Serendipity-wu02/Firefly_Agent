...
import { createSandboxProfileId, type SandboxDecision, type SandboxScope } from "../../../dist/main/shared/sandbox-types.js";
import {
...
function createRequest(): CapabilityRequest<{ path: string }> {
  return {
...
function createSandboxDecision(): SandboxDecision {
...
function createApprovalDecision(): ApprovedApprovalDecision {
  const service = new ApprovalService({
    now: () => 1_000,
    createRequestId: () => createApprovalRequestId("approval-integration-1"),
  });
  ...
  const record = service.approve(pending.request.approvalRequestId, {
    kind: "filesystem",
    path: "C:\\workspace\\nested",
    access: "read",
  });
  assert.ok(record.decision?.approved);
...
test("D. ApprovalService rejects a grant wider than the Sandbox effective scope", () => {
  const service = new ApprovalService({
    now: () => 1_000,
    createRequestId: () => createApprovalRequestId("approval-widening-1"),
  });
  const pending = service.createPending({
...
test("I. An approval grant without a valid Sandbox context is rejected", () => {
  const deniedSandbox: SandboxDecision = {
    allowed: false,
...
test("J. A SubAgent declaration alone does not create a binding or authorization", () => {
  const subAgentId = createSubAgentId("integration-worker");
  ...
  const taskService = new SubAgentTaskService({
    registry,
     createTaskId: () => createSubAgentTaskId("integration-task-1"),
  });
...
test("K. Foundation components do not import or invoke ToolExecutionEngine", () => {
  for (const directory of ["capabilities", "sandbox", "approval"]) {
...
test("O. SubAgent runtime has no Character, Memory, RAG, TTS, or Live2D dependency", () => {
  const directory = path.join(projectRoot, "src", "main", "orchestrator", "subagents");

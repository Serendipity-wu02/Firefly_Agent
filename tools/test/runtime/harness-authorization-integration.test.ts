...
import type { ToolCall, ToolCallResult, ToolContext } from "../../../dist/main/shared/tool-types.js";
import type { ToolExecutionContext } from "../../../dist/main/main/orchestrator/tools/execution/tool-execution-context.js";
import type { ToolPolicyConfig } from "../../../dist/main/main/orchestrator/tools/execution/tool-policy.js";
...
const pilotScope = { kind: "desktop", target: "QQMusic" } as const;
...
async function waitForPending(approvalService: ApprovalService, timeoutMs = 1000): Promise<ApprovalRecord> {
  return waitForValue<ApprovalRecord | undefined>(() => approvalService.listPending()[0], timeoutMs);
}
...
interface FixtureOptions {
  ...
  readonly pilotOutput?: string;
  readonly toolPolicy?: Partial<ToolPolicyConfig>;
  readonly now?: () => number;
  readonly registerCapability?: boolean;
  readonly registerBinding?: boolean;
  readonly sandboxAllowed?: boolean;
  readonly approvalTtlMs?: number;
}

interface FixtureState {
  pilotExecutions: number;
  legacyExecutions: number;
  engineCalls: number;
  engineContexts: Array<{ call: import("../../../dist/main/shared/tool-types.js").ToolCall; context: ToolExecutionContext }>;
}

function createFixture(options: FixtureOptions = {}) {
  let profile: PermissionProfile = options.profile ?? "RESTRICTED_SCOPE";
  ...
  engine.executeToolCall = async (
    call: import("../../../dist/main/shared/tool-types.js").ToolCall,
  ...
  return {
    ...
    pipeline,
    registry,
    setProfile: (value: PermissionProfile) => {
      profile = value;
    },
    state,
  };
...
function createHarness(
  ...
  options: Partial<FireflyHarnessOptions> = {},
) {
...
function createToolContext(overrides: Partial<ToolContext & ToolExecutionContext> = {}): ToolExecutionContext {
  return {
...
function toolCall(id = "pilot-call"): import("../../../dist/main/shared/tool-types.js").ToolCall {
...
test("A-J. The music_status pilot follows Capability, Sandbox, Bridge, Engine, and canonical Harness writeback", async () => {
  ...
  const firstEngineContext = fixture.state.engineContexts[0];
  assert.ok(firstEngineContext);
  assert.ok(firstEngineContext.context.upstreamAuthorization);
  assert.equal(firstEngineContext.context.upstreamAuthorization.capabilityId, pilotCapabilityId);
  assert.equal(firstEngineContext.context.upstreamAuthorization.toolId, pilotToolId);
  assert.equal(firstEngineContext.context.upstreamAuthorization.authorization.type, "sandbox-only");
...
test("K-M, R-T. ASK_EVERY_TIME waits in the Harness, resumes one logical call, and avoids a second ToolPolicy confirmation", async () => {
  ...
  const terminalRecords: ApprovalRecord[] = [];
  ...
  assert.equal(fixture.state.pilotExecutions, 1);
  assert.equal(fixture.state.engineCalls, 1);
  const approvedEngineContext = fixture.state.engineContexts[0];
  assert.ok(approvedEngineContext);
  assert.ok(approvedEngineContext.context.upstreamAuthorization);
  assert.equal(approvedEngineContext.context.upstreamAuthorization.approvalRequirement, "required");
  assert.equal(approvedEngineContext.context.upstreamAuthorization.authorization.type, "approval-grant");
...
test("U-Z. A pending pilot forms a same-round barrier; deferred siblings are written in order and re-planned in the next round", async () => {
  ...
  const firstBarrierContext = fixture.state.engineContexts[0];
  const secondBarrierContext = fixture.state.engineContexts[1];
  assert.ok(firstBarrierContext);
  assert.ok(secondBarrierContext);
  assert.ok(firstBarrierContext.context.upstreamAuthorization);
  assert.equal(firstBarrierContext.context.upstreamAuthorization.toolId, pilotToolId);
  assert.equal(secondBarrierContext.context.upstreamAuthorization, undefined);
...
test("AF-AN. The adapter keeps execution, state, UI, and architecture ownership canonical", () => {
  ...
  assert.equal((compositionSource.match(/new ToolExecutionEngine\(/g) ?? []).length, 1);
  assert.equal((compositionSource.match(/new AgentEventBus\(/g) ?? []).length, 1);
  assert.equal((compositionSource.match(/new FireflyAgentCore\(/g) ?? []).length, 1);
  assert.equal(compositionSource.includes("new FireflyHarness("), false);
  assert.ok(compositionSource.includes("new AuthorizedInvocationBridge("));
  assert.ok(compositionSource.includes("toolId: \"music_status\""));

  const typeGuard = spawnSync(
    process.execPath,
    ["--experimental-strip-types", path.join(projectRoot, "tools", "verify", "verify-typescript-production.mts")],
    { cwd: projectRoot, encoding: "utf8" },
  );
  assert.equal(typeGuard.status, 0, typeGuard.stderr || typeGuard.stdout);
  const architectureGuard = spawnSync(
    process.execPath,
    ["--experimental-strip-types", path.join(projectRoot, "tools", "verify", "verify-architecture.mts")],
    { cwd: projectRoot, encoding: "utf8" },
  );

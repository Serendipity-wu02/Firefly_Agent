...
import type { ApprovalRequest } from "../../../dist/main/shared/approval-types.js";
...
function createExecutor({ sessionAvailable = true, commandGate = false }: {
  ...
  calls: GsmtcAction[];
  commandStarted: Promise<void>;
  controlCalls: () => GsmtcAction[];
} {
  const calls: GsmtcAction[] = [];
  let resolveCommandStarted: (value: void | PromiseLike<void>) => void;
...
function createFixture({
  ...
  const controlTool = toolRegistry.get("music_control");
  if (!controlTool) throw new Error("music_control test tool was not registered");
...
function context(callId: string, signal: AbortSignal = new AbortController().signal): ToolExecutionContext {
  return {
...
function controlCall(id: string, action: string): ToolCall {
  return { id, name: "music_control", arguments: { action } };
}

function resultBody(result: { output: string }): Record<string, unknown> {
  return JSON.parse(result.output);
}

async function approveOnPending(fixture: ReturnType<typeof createFixture>) {
  return {
    onPendingApproval: (request: ApprovalRequest) => {
...
test("A-F. music.control route, binding, QQMusic Sandbox target, and fixed tool surface", async () => {
  ...
  try {
    assert.equal(fixture.adapter.handles({ id: "handles-control", name: "music_control", arguments: {} }), true);
    assert.equal(fixture.adapter.handles({ id: "handles-status", name: "music_status", arguments: {} }), true);
    assert.equal(fixture.adapter.handles({ id: "handles-search", name: "music_search", arguments: {} }), false);
    assert.equal(fixture.adapter.handles({ id: "handles-recommend", name: "music_recommend", arguments: {} }), false);
    assert.equal(fixture.adapter.handles({ id: "handles-play", name: "music_play", arguments: {} }), false);
  ...
  const script = fs.readFileSync(
    path.join(projectRoot, "src", "main", "music", "scripts", "qqmusic_gsmtc.ps1"),
    "utf8",
  );
  assert.match(script, /SourceAppUserModelId/);
  assert.match(script, /QQMusic/);
  const bridgeSource = fs.readFileSync(
    path.join(projectRoot, "src", "main", "music", "qqmusic-desktop-bridge.ts"),
    "utf8",
  );
  assert.doesNotMatch(bridgeSource, /src.*main.*music.*scripts/);
  } finally {
...
test("G-K. Supported play, pause, next, previous, and toggle actions route to GSMTC once", async () => {
  for (const [action, expectedBridgeAction] of [
    ...
    try {
      ...
      assert.equal(body.target, "QQMusic");
      assert.equal(body.commandSubmission, "accepted");
      assert.equal(body.playerStateObservation, "unchanged");
      assert.equal((body.observedState as Record<string, unknown>).available, true);
      assert.equal(
        ((body.observedState as Record<string, unknown>).track as Record<string, unknown>).name,
        "测试歌曲",
      );
...
test("L. Unsupported commands are rejected before external control", async () => {
  ...
  try {
    ...
    assert.equal(resultBody(result).error, "COMMAND_UNSUPPORTED");
...
test("M-N. The four permission schemes enforce the canonical control policy", async () => {
  for (const profile of ["RESTRICTED_SCOPE", "ASK_EVERY_TIME", "FULL_ACCESS"] as PermissionProfile[]) {
    ...
    try {
      ...
      const pendingApproval = pendingRequest;
      const pendingRecord = fixture.approvalService.get(pendingApproval.approvalRequestId);
      assert.ok(pendingRecord);
      assert.equal(pendingRecord.state, "pending");
      fixture.approvalService.approve(pendingApproval.approvalRequestId);
...
test("O-Q. Approval executes once, denial executes zero times, and an unavailable QQMusic session never falls back", async () => {
  ...
  try {
    ...
    assert.equal(resultBody(result).error, "PLAYER_NOT_FOUND");
...
test("R-S. Cancel, stale correlation, and duplicate approval cannot dispatch a second command", async () => {
  ...
  assert.throws(() => duplicate.approvalService.approve(createApprovalRequestId("qqmusic-control-1")));
  ...
  const mismatch = await fixture.controlTool.execute(
    { action: "next" },
    {
      ...
      upstreamAuthorization: {
        capabilityId: controlCapabilityId,
         requestId: createCapabilityRequestId("wrong-target-request"),
        requester: { type: "main-agent", id: "firefly-harness" },
        toolId: "music_control",
        ...
        authorization: {
          type: "approval-grant",
           approvalRequestId: createApprovalRequestId("wrong-target-approval"),
          grantLifetime: "once",
        },
      },
    },
...
test("W-X. music_status remains authorized read-only and discovery remains unregistered", async () => {
  ...
  try {
    ...
    assert.deepEqual(fixture.controlCalls(), []);
    assert.equal(fixture.capabilityRegistry.has(searchCapabilityId), false);
     assert.equal(fixture.adapter.handles({ id: "handles-search-regression", name: "music_search", arguments: {} }), false);
  } finally {
    await fixture.service.shutdown();
  }
});

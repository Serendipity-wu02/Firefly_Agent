...
import { normalizeGptsovitsText, synthesizeGptsovits } from "../../../dist/main/main/tts/engines/gptsovits-engine.js";
...
import { buildTtsCacheKey, TTS_CACHE_VERSION, versionTtsCacheKey } from "../../../dist/main/main/tts/tts-cache-key.js";
import { TtsPlaybackOwnership } from "../../../dist/main/main/tts/playback-owner.js";
import {
...
import type { GptsovitsConfig, TtsSettings } from "../../../dist/main/shared/tts-types.js";
import type { TtsStartResult } from "../../../dist/main/shared/tts-session.js";
import { getTtsTextIntegrity } from "../../../dist/main/shared/tts-text-integrity.js";

const rootDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../..");

interface MockGptsovitsOptions {
  shouldFail?: boolean;
  delayMs?: number;
}

interface MockGptsovitsServer {
  baseUrl: string;
  server: http.Server;
  getRequestCount(): number;
  getLastPayload(): Record<string, unknown>;
  close(): Promise<void>;
}
...
function requireReady(result: TtsStartResult): Extract<TtsStartResult, { status: "ready" }> {
  if (result.status !== "ready") {
    throw new Error("Expected ready TTS result, received " + result.status);
  }
  return result;
}
...
function createMockGptsovitsServer(options: MockGptsovitsOptions = {}): Promise<MockGptsovitsServer> {
  let requestCount = 0;
  ...
  return new Promise<MockGptsovitsServer>((resolve, reject) => {
    server.listen(0, "127.0.0.1", () => {
      const address = server.address();
      if (!address || typeof address === "string") {
        reject(new Error("Mock GPT-SoVITS server did not expose an address"));
        return;
      }
...
test("4. Service Unavailable / Model Failure & Graceful Degradation", async () => {
  ...
  const deadConfig: GptsovitsConfig = {
  ...
  await assert.rejects(
    async () => {
      ...
      (err: unknown) => {
      assert.ok(errorMessage(err).includes("GPT-SoVITS 服务连接失败"));
      return true;
    },
  );
});
...
test("5. Server Error 500 Diagnosis", async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (err: unknown) => {
        assert.ok(errorMessage(err).includes("HTTP 500"));
        return true;
      },
    );
  } finally {
...
test("6b. TTS cache key includes effective seed and the new schema version", async () => {
  ...
  try {
    const dispatcher = new FireflyTtsDispatcher(new TtsCache(cacheDir));
    const createSettings = (seed: number): TtsSettings => ({
      engine: "gptsovits",
      speed: 1.0,
      volume: 1.0,
      voiceProfile: "firefly-v2proplus",
    ...
    assert.equal(requireReady(seed15Result).cacheKey, expectedVersionedKey);
    assert.notEqual(requireReady(seed15Result).cacheKey, baseCacheKey);
    assert.notEqual(requireReady(seed5Result).cacheKey, requireReady(seed15Result).cacheKey);
    assert.notEqual(requireReady(seed15Result).cacheKey, versionTtsCacheKey(baseCacheKey, "tts-lexical-integrity-v2"));
    assert.equal(mockServer.getLastPayload().seed, 15);
    assert.equal(mockServer.getLastPayload().text_split_method, "cut5");
...
test("8. Playback lifecycle guards, cleanup, MouthSync failsafe, and ownership IPC are canonical", () => {
  ...
  const ttsIpcSource = fs.readFileSync(path.join(rootDir, "src", "main", "tts", "tts-ipc.ts"), "utf8");

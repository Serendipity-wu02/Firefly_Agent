...
const rootScope: SandboxScope = {
  kind: "filesystem",
  path: "C:\\workspace",
  access: "read",
};

function createFixture(requirement: ApprovalRequirement = "none", registerBinding = true) {
  let now = 1_000;
...
function createRequest(overrides: Partial<CapabilityRequest<{ path: string }>> = {}): CapabilityRequest<{ path: string }> {
  return {
...
function requireAuthorized(
  result: CapabilityAuthorizationOutcome,
) {
  ...
  assert.throws(
    () => createAuthorizedCapabilityInvocation({
      ...
      binding: { capabilityId, toolId },
       sandboxDecision: { allowed: true, effectiveScope: rootScope } satisfies SandboxDecision,
     ...
     (error) => error instanceof AuthorizedInvocationError && error.code === "APPROVAL_SCOPE_WIDENED",
  );
});

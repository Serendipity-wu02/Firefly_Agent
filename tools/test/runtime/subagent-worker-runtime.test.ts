...
import type {
  SubAgentTaskConstraints,
  SubAgentTaskInput,
} from "../../../dist/main/shared/subagent-types.js";
import type { ToolCall } from "../../../dist/main/shared/tool-types.js";
...
interface WorkerFixtureOptions {
  ...
  constraints?: Partial<SubAgentTaskConstraints>;
  approvalTtlMs?: number;
  statusExecute?: (args: Record<string, unknown>, context?: import("../../../dist/main/shared/tool-types.js").ToolContext) => Promise<string>;
}
...
interface WorkerFixtureState {
  ...
  lastStatusContext?: import("../../../dist/main/shared/tool-types.js").ToolContext;
}

type TaskOverrides = Omit<Partial<SubAgentTaskInput>, "constraints"> & {
  constraints?: Partial<SubAgentTaskConstraints>;
};

function requestAt(state: WorkerFixtureState, index: number): ChatCompletionRequest {
...
function createFixture({
  ...
  let taskSequence = 0;
  const state: WorkerFixtureState = {
    statusExecutions: 0,
    controlExecutions: 0,
    engineCalls: [],
    requests: [],
  ...
  function createTask(extra: TaskOverrides = {}) {
    const { constraints: extraConstraints, ...taskOverrides } = extra;
...
test("worker success uses the canonical Harness and authorized status path", async () => {
  ...
  const firstTools = requestAt(fixture.state, 0).tools;
  assert.ok(firstTools);
  assert.deepEqual(
    firstTools.map((entry) => entry.function.name),
    [statusToolId],
  );
  ...
  const upstreamAuthorization = firstEngineCall.context.upstreamAuthorization;
  assert.ok(upstreamAuthorization);
  assert.deepEqual(upstreamAuthorization.requester, {
    type: "subagent",
  ...
  const systemPrompt = requestAt(fixture.state, 0).messages[0]?.content;
  ...
  assert.equal(requestAt(fixture.state, 0).messages.length, 2);
  assert.equal(
...
test("worker tool surface rejects an undeclared call before adapter or engine execution", async () => {
  ...
  assert.ok(lastMessage(requestAt(fixture.state, 1)).content.includes("worker_capability_not_declared"));
});
...
test("worker approval denial produces a typed observation and zero tool execution", async () => {
  ...
  assert.equal(fixture.state.engineCalls.length, 0);
  assert.ok(lastMessage(requestAt(fixture.state, 1)).content.includes("APPROVAL_DENIED"));
});
...
test("worker approval expiry produces a typed observation and zero tool execution", async () => {
  ...
  assert.equal(fixture.state.statusExecutions, 0);
  assert.equal(fixture.state.engineCalls.length, 0);
  const expiredApproval = fixture.approvalService.get(approval.request.approvalRequestId);
  assert.ok(expiredApproval);
  assert.equal(expiredApproval.state, "expired");
  assert.ok(lastMessage(requestAt(fixture.state, 1)).content.includes("APPROVAL_EXPIRED"));
});
...
test("worker same-round approval barrier defers later calls", async () => {
  ...
  assert.equal(fixture.state.statusExecutions, 1);
  assert.equal(fixture.state.engineCalls.length, 1);
  assert.ok(lastMessage(requestAt(fixture.state, 1)).content.includes("deferred_after_approval"));
});
...
test("worker timeout is reported as a structured failure and never becomes success", async () => {
  ...
  if (!record.result.ok) assert.equal(record.result.error.code, "TIMEOUT");
});
...
test("worker tool budget is forwarded to the canonical execution engine", async () => {
  ...
  assert.equal(fixture.state.statusExecutions, 1);
  assert.ok(lastMessage(requestAt(fixture.state, 1)).content.includes("budget_exceeded"));
});
...
test("worker step budget stops the canonical loop at the task limit", async () => {
  ...
  assert.equal(record.result.ok, false);
  if (!record.result.ok) {
    assert.equal(record.result.error.code, "BUDGET_EXHAUSTED");
    assert.deepEqual(record.result.error.terminationReason, {
...
test("nested worker tasks are rejected before the canonical loop starts", async () => {
  ...
  await assert.rejects(
    ...
    (error) => error instanceof Error && "code" in error && error.code === "INVALID_DELEGATION_DEPTH",
  );
  const childRecord = fixture.taskService.get(child.task.taskId);
  assert.ok(childRecord);
  assert.equal(childRecord.state, "pending");
});

...
import type { AgentEventType } from "../../../dist/main/shared/agent-types.js";
...
type MockLlmHandlers = {
  ...
  onGenerate?: (
    ...
    callCount: number,
  ) => Promise<ChatCompletionResponse> | ChatCompletionResponse;
};

function createControllableMockLlm(handlers: MockLlmHandlers = {}): {
  provider: IFireflyLlmProvider;
  calls: Array<{ request: ChatCompletionRequest; aborted: boolean | undefined }>;
} {
  const calls: Array<{ request: ChatCompletionRequest; aborted: boolean | undefined }> = [];
...
test("1. FireflyAgentCore Structure & Static Decoupling Audit", () => {
  ...
  // FireflyAgentCore is the public facade and delegates the unique loop.
  assert.ok(coreSource.includes("FireflyHarness"), "Core must compose FireflyHarness");
  assert.ok(coreSource.includes("this.harness.run"), "Core must delegate run to FireflyHarness");
  assert.equal(coreSource.includes("while ("), false, "Core must not own an Agent loop");
  assert.equal(coreSource.includes("executeToolCall"), false, "Core must not execute tools directly");
  ...
  // The application composition root now delegates concrete construction to
  // the default dependency graph; the Electron entry must not own it.
  ...
  assert.equal(indexSource.includes("new FireflyAgentCore("), false, "index.ts must not instantiate FireflyAgentCore");
  assert.ok(dependenciesSource.includes("new FireflyAgentCore("), "default dependencies must instantiate FireflyAgentCore");
  assert.equal(dependenciesSource.includes("new FireflyHarness("), false, "default dependencies must NOT instantiate FireflyHarness");
});

// 2. Normal Chat Conversation & Event Sequence
test("2. Normal Chat: Message History & Exact Event Sequence", async () => {
  const emittedEvents: AgentEventType[] = [];
...
test("7. Cancellation: In-Flight LLM, Tool Execution, and Inter-Round Cancel", async () => {
  ...
  const providerA: IFireflyLlmProvider = {
    id: "slow-llm",
    name: "Slow LLM",
    capabilities: { supportsNativeToolCalling: true, supportsStreaming: false },
    getConfig: () => ({ provider: "local", baseUrl: "", apiKey: "", model: "mock", temperature: 0.7, enableStreaming: false }),
...
test("8. Timeout: Total Timeout Aborts Run with 'timeout' Status", async () => {
  const toolRegistry = new FireflyToolRegistry();
  const hangingProvider: IFireflyLlmProvider = {
    id: "hanging-llm",
    name: "Hanging LLM",
    capabilities: { supportsNativeToolCalling: true, supportsStreaming: false },
    getConfig: () => ({ provider: "local", baseUrl: "", apiKey: "", model: "mock", temperature: 0.7, enableStreaming: false }),
...
test("13. Final Answer Contract: Completed result retains conversation identity, timing, transcript, and text", async () => {
  const { provider } = createControllableMockLlm({
    onGenerate: async () => ({ message: { role: "assistant", content: "最终答案。" } }),
  });
  const core = new FireflyAgentCore({ provider, toolRegistry: new FireflyToolRegistry() });

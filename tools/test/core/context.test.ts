...
import type { SlotRenderContext } from "../../../dist/main/main/orchestrator/context/context-slots.js";
...
test("1. TokenMeter: estimate vs actual token calculation with injectable tokenizer", () => {
  ...
  const customTokenizer = {
    countTokens: (str: string) => str.length * 10,
  };
...
test("2. SystemPromptBuilder: Firefly persona, semantic state, and action catalog", () => {
  ...
  assert.ok(prompt.includes("【当前内在状态】"), "Must include semantic state");
  assert.ok(prompt.includes("关切担忧"), "Must reflect the current request in semantic state");
  assert.ok(!prompt.includes("状态基线（兼容）"), "Must not project retired numeric state");
...
test("3. ContextSlots: System, State, Memory, and RAG slot rendering & token estimation", async () => {
  const meter = new TokenMeter();
  ...
  assert.ok(stateSlot.render(ctx).includes("【当前内在状态】"));
  assert.equal(await memorySlot.render(ctx), "用户喜欢橡木蛋糕卷");
  assert.ok((await ragSlot.render(ctx)).includes("匹诺康尼的梦境设定"));

...
interface ModelFileReferences {
  ...
  Textures: string[];
  Physics: string;
  Motions: Record<string, MotionReference[]>;
...
function readModel(): ModelDocument {
  return JSON.parse(fs.readFileSync(MODEL_JSON_PATH, "utf-8")) as ModelDocument;
}
...
test("5. Motion Paths: All motions in Idle and Tap groups resolve 100% to models/Motions/", () => {
  ...
  for (const [group, list] of Object.entries(motions)) {
    for (let i = 0; i < list.length; i++) {
      ...
      const content = JSON.parse(fs.readFileSync(motionPath, "utf-8")) as MotionDocument;
...
test("6. Expression Paths: All 11 expressions resolve 100% to models/Expressions/", () => {
  ...
  for (let i = 0; i < expressions.length; i++) {
    ...
    const content = JSON.parse(fs.readFileSync(expPath, "utf-8")) as ExpressionDocument;

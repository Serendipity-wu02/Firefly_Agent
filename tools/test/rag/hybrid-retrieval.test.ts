...
import type { LexicalCandidate } from "../../../dist/main/main/rag/lexical-retriever.js";
import type { VectorCandidate } from "../../../dist/main/main/rag/vector-retriever.js";
...
test("5. Lexical / Vector Weighting: Custom weights accurately steer final scoring", async () => {
  ...
  const lexCandidates: LexicalCandidate[] = [{ chunk: sampleChunks[0], lexicalScore: 0.9 }];
  const vecCandidates: VectorCandidate[] = [{ chunk: sampleChunks[0], vectorScore: 0.4 }];
...
test("6. Canonical Priority: Higher canonical priority breaks ties when relevance is equal", () => {
  ...
  const lexCandidates: LexicalCandidate[] = [
...
test("9. Reranking: Combines all dimensions into normalized final score in [0.0, 1.0]", () => {
  const reranker = new KnowledgeReranker();
  const lexCandidates: LexicalCandidate[] = [{ chunk: sampleChunks[2], lexicalScore: 0.8 }];
  const vecCandidates: VectorCandidate[] = [{ chunk: sampleChunks[2], vectorScore: 0.7 }];
...
test("11. Deterministic Ordering: Permuting candidate order yields identical ranked results", () => {
  const reranker = new KnowledgeReranker();
  const c1: LexicalCandidate = { chunk: sampleChunks[0], lexicalScore: 0.5 };
  const c2: LexicalCandidate = { chunk: sampleChunks[1], lexicalScore: 0.5 };
...
test("12. Duplicate Candidate Merge: Chunk appearing in both lexical and vector streams is merged", () => {
  const reranker = new KnowledgeReranker();
  const lex: LexicalCandidate[] = [{ chunk: sampleChunks[0], lexicalScore: 0.6 }];
  const vec: VectorCandidate[] = [{ chunk: sampleChunks[0], vectorScore: 0.7 }];
...
test("20. Adversarial Irrelevant High-Priority Source: Irrelevant chunk with 1.0 priority is rejected by relevance gating", () => {
  const irrelevantP1Chunk: KnowledgeChunk = {
  ...
  const relevantWikiChunk: KnowledgeChunk = {

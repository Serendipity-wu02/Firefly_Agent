...
import type { KnowledgeChunk, KnowledgeManifest } from "../../../dist/main/main/rag/rag-types.js";

const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "firefly-rag-vector-"));
const testOutputDir = path.join(testRoot, "vector-store");
...
test("13. Source Modification: Force reindex replaces existing vector values", async () => {
  ...
  const chunk: KnowledgeChunk = { id: "c1", documentId: "d1", sourceUri: "s1", chunkIndex: 0, chunkType: "atomic_fact", text: "原版文本", tokenEstimate: 5, entityNames: [], keywords: [], canonicalPriority: 1, metadata: { perspective: "first_person", sourceFile: "s1", characters: [], verified: true } };
...
test("14. Deleted Source Cleanup: syncWithManifest safely removes obsolete vector records", async () => {
  ...
  const emptyManifest: KnowledgeManifest = {
    version: "test",
    generatedAt: 1,
    totalSources: 0,
    totalDocuments: 0,
  ...
  const deletedCount = await indexService.syncWithManifest(emptyManifest, [chunks[0]]);
...
test("20. In-Memory Isolation: Vector operations do not mutate unrelated disk state", async () => {
  const sentinelPath = path.join(testRoot, "unrelated-state.json");

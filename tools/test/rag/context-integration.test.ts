...
import type { KnowledgeChunk } from "../../../dist/main/main/rag/rag-types.js";
import type { ScoredKnowledgeChunk, ScoreBreakdown } from "../../../dist/main/main/rag/retrieval-types.js";
import type { IKnowledgeRetrieverAdapter, IKnowledgeProjectorAdapter } from "../../../dist/main/main/orchestrator/context/context-slots.js";
...
const scored = (chunk: KnowledgeChunk, finalScore = 0.9, breakdown = EMPTY_BREAKDOWN): ScoredKnowledgeChunk => ({
  chunk,
  finalScore,
  breakdown,
});
...
test("2. Query Propagation: User prompt passed into RagSlot is forwarded to retriever", async () => {
  let capturedQuery = "";
  const mockRetriever: IKnowledgeRetrieverAdapter = {
    retrieve: async (q: { queryText?: string }) => {
...
test("3. HybridRetriever Invocation: Renders retrieved items through projector", async () => {
  ...
  const mockRetriever: IKnowledgeRetrieverAdapter = {
    retrieve: async () => ({
      items: [
        scored(sampleChunks[0], 0.9, { lexicalScore: 0.9, vectorScore: 0.8, entityScore: 1, canonicalScore: 0.95, timelineScore: 0.5 }),
      ],
    }),
  };
  ...
  const slot = new RagSlot({
    ...
    projector: {
      project: (items, cfg) => projector.project(items as readonly ScoredKnowledgeChunk[], cfg),
    } satisfies IKnowledgeProjectorAdapter,
  });
...
test("4. Result Projection: Formats clean structured markdown lines", () => {
  const projector = new KnowledgeProjector();
  const items = [
    scored(sampleChunks[1], 0.85, { lexicalScore: 0.8, vectorScore: 0.7, entityScore: 1, canonicalScore: 0.85, timelineScore: 0.5 }),
  ];
...
test("11. Read-Only Isolation: RagSlot rendering does not mutate unrelated disk state", async () => {
  const sentinelPath = path.join(os.tmpdir(), "firefly-rag-slot-sentinel.json");
  fs.writeFileSync(sentinelPath, "unchanged", "utf-8");
  const projector = new KnowledgeProjector();
  const slot = new RagSlot({
    retriever: { retrieve: async () => ({ items: [scored(sampleChunks[0])] }) },
    projector: { project: (items) => projector.project(items as readonly ScoredKnowledgeChunk[]) },
  });
...
test("18. RAG Projection Does Not Mutate Source Corpus: Raw chunk objects remain unmodified", async () => {
  ...
  const projector = new KnowledgeProjector();
  projector.project([scored(sampleChunks[0])]);

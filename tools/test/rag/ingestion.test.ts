...
import type { KnowledgeSource } from "../../../dist/main/main/rag/rag-types.js";

const projectRoot = path.resolve(import.meta.dirname, "../../..");
const resourcesRoot = path.join(projectRoot, "src", "main", "character", "resources");
const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "firefly-rag-ingestion-"));
const testOutputDir = path.join(testRoot, "knowledge");
...
test("14. Output Isolation: saveToFile=false does not write an ingestion manifest", async () => {
  const isolatedOutputDir = path.join(testRoot, "no-save");
  const service = new KnowledgeIngestionService({ resourcesRoot, outputDir: isolatedOutputDir });
  await service.ingestAll({ saveToFile: false });
  assert.equal(fs.existsSync(path.join(isolatedOutputDir, "manifest.json")), false);
});

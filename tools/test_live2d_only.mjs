...
test("1. Live2D Model Path: Confirms presence of models directory and main config", () => {
  assert.ok(fs.existsSync(MODELS_DIR), "src/renderer/models/ must exist");
...
test("9. Normal Asset Audit: src/renderer/models/normal/ is 100% removed from disk", () => {
  const normalDir = path.join(PROJECT_ROOT, "src", "renderer", "models", "normal");
  assert.ok(!fs.existsSync(normalDir), "src/renderer/models/normal directory must NOT exist");
});

...
const WINDOW_CLASS: PCWSTR = w!("FireflyScreenshotRuntimeWindow");
...
fn run_message_loop(
    ...
    let capture: Box<dyn CaptureBackend> = match DxgiCaptureBackend::new(&display, false) {
        Ok(c) => {
            eprintln!("firefly-screenshot: using DXGI capture backend");
            Box::new(c)
        }
        Err(error) => {
            eprintln!("firefly-screenshot: DXGI init failed ({error}), falling back to GDI");
    ...
    loop {
        ...
        match pump_capture_if_idle(app_state.active.is_some(), |timeout_ms| {
            ...
            Err(error) => {
                eprintln!(
                    "firefly-screenshot: idle DXGI refresh failed ({error}); rebuilding capture"
                );
...
impl OverlayApp {
            ...
            FrozenFrame::Gpu(gpu) => {
                ...
                match renderer.upload_frozen_gpu(gpu, overlay.hwnd(), display) {
                    ...
                    Err(error) => {
                        ...
                        eprintln!(
                            "firefly-screenshot: GPU upload failed ({error}), falling back to GDI for this session"
                        );
    ...
    fn process_overlay_action(&mut self, event_tx: &std::sync::mpsc::Sender<Event>) {
        ...
        if action == OverlayAction::DisplayChanged {
            ...
            if let Err(error) = self.recover_capture() {
                eprintln!("firefly-screenshot: display-change recovery failed: {error}");
            }
            return;
        }
    ...
    fn recover_capture(&mut self) -> Result<(), HelperError> {
        ...
        match self.capture.recover(&display) {
            ...
            Err(error) => {
                eprintln!(
                    "firefly-screenshot: {} recovery failed ({error}), falling back to GDI",
                    self.capture.name()
                );
    ...
    fn freeze_with_recovery(&mut self) -> Result<FrozenFrame, HelperError> {
        ...
        let frame = match first {
            ...
            Err(error) if self.capture.name() == "dxgi" => {
                eprintln!(
                    "firefly-screenshot: DXGI freeze failed ({error}); rebuilding before retry"
                );
                self.overlay.detach_renderer();
                self.renderer.reset_gpu_resources();
                if let Err(rebuild_error) = self.capture.recover(&self.display) {
                    eprintln!(
                        "firefly-screenshot: DXGI rebuild failed ({rebuild_error}); using GDI"
                    );
                    self.capture = Box::new(GdiCaptureBackend::new()?);
                }
                match self.capture.freeze(&self.display) {
                    ...
                    Err(retry_error) if self.capture.name() == "dxgi" => {
                        eprintln!(
                            "firefly-screenshot: rebuilt DXGI freeze failed ({retry_error}); using GDI"
                        );
                        self.capture = Box::new(GdiCaptureBackend::new()?);
        ...
        if self.capture.should_fallback_to_gdi() {
            eprintln!(
                "firefly-screenshot: DXGI cursor policy requires GDI for the current session"
            );
            self.capture = Box::new(GdiCaptureBackend::new()?);
    ...
    fn commit(&mut self, event_tx: &Sender<Event>) {
        ...
        eprintln!(
            "firefly-screenshot: committed selection={}x{} at ({},{}) display={:?} rotation={:?} backend={} diag={:?}",
            selection.width,
            selection.height,
            selection.x,
            selection.y,
        ...
        let clipboard_written = write_cf_dibv5(self.overlay.hwnd(), &frame)
            ...
            .unwrap_or_else(|error| {
                eprintln!("firefly-screenshot: clipboard write failed: {error}");
                false
            });
        ...
        match lock_registry(&self.requests) {
            Ok(mut registry_guard) => {
                if let Err(error) = registry_guard.capture_released(
                    ...
                    eprintln!("firefly-screenshot: registry capture_released failed: {error}");
                }
            }
            Err(error) => {
                eprintln!(
                    "firefly-screenshot: registry capture_released lock unavailable: {error}"
                );
            }
        }
...
fn lock_registry(
    ...
    match registry.lock() {
        Ok(guard) => Ok(guard),
        Err(poison) => {
            eprintln!("firefly-screenshot: requests mutex poisoned, recovering");
    ...
    match lock_registry(registry) {
        Ok(mut registry_guard) => {
            ...
            if let Err(error) = outcome {
                eprintln!(
                    "firefly-screenshot: requests finalize ({mode}) for {request_id} failed: {error}"
                );
            }
        }
        Err(_) => {
            eprintln!("firefly-screenshot: requests finalize lock unavailable");
        }
    }
}

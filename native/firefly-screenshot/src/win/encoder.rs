    ...
    match registry.lock() {
        Ok(mut registry_guard) => {
            ...
            if let Err(error) = outcome {
                eprintln!(
                    "firefly-screenshot: encoder finalize ({mode}) for {request_id} failed: {error}"
                );
            }
        }
        Err(poison) => {
            eprintln!(
                "firefly-screenshot: encoder finalize lock unavailable for {request_id}: {poison}"
            );
        }
    }
}

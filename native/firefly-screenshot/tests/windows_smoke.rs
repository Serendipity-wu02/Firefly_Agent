...
impl Helper {
    ...
    fn spawn_with_output_dir(parent_pid: u32, protocol_version: u32, output_dir: PathBuf) -> Self {
        let mut child = Command::new(env!("CARGO_BIN_EXE_firefly-screenshot"))
            .args([
                "--output-dir",
    ...
    fn overlay_hwnd(&self) -> HWND {
        find_process_window(
            ...
            None,
            w!("FireflyScreenshotOverlayWindow"),
        )
    }
    ...
    fn suspend_ui_thread(&self) -> SuspendedThread {
        let hwnd = find_process_window(
            self.process.child.id(),
            Some(HWND_MESSAGE),
            w!("FireflyScreenshotRuntimeWindow"),
        );
        let mut pid = 0;
...
fn absolute_output_dir() -> PathBuf {
    std::env::temp_dir().join("firefly-screenshot-smoke")
}
...
fn oversized_input_emits_recoverable_error_and_keeps_running() {
    ...
    stdin
        .write_all(&vec![
            b'x';
            firefly_screenshot::protocol::MAX_NDJSON_LINE_BYTES + 1
        ])
        .unwrap();
    stdin.write_all(b"\n").unwrap();
    stdin.flush().unwrap();

...
use firefly_screenshot::win::capture::{CaptureBackend, CpuBgraFrame, FrozenFrame, RefreshOutcome};
use firefly_screenshot::win::capture_gdi::GdiCaptureBackend;
use firefly_screenshot::win::display::query_primary_display;

#[test]
fn gdi_freeze_produces_full_primary_bounds_bgra() {
    firefly_screenshot::win::display::set_dpi_awareness()
...
fn gdi_freeze_returns_invalid_display_for_overflowing_dimensions() {
    ...
    let display = firefly_screenshot::win::display::DisplayInfo {
        bounds: firefly_screenshot::geometry::RectI {
            x: 0,
            y: 0,
        ...
        dpi: 96.0,
        rotation: firefly_screenshot::geometry::DisplayRotation::Identity,
        is_primary: true,
    };
    ...
    assert!(
        matches!(
            err,
            firefly_screenshot::error::HelperError::InvalidDisplay(_)
        ),

---
name: firefly-plugin-dev
description: 当用户想为 Firefly 开发插件（扩展流萤的工具、做插件弹窗、接入渠道）、调试插件报错，或询问插件怎么写时使用。涉及 Firefly 本体源码的功能开发不使用本 Skill。
version: 1.3.0
effectKind: mutation
modes:
  - code
...
本 Skill 指导如何为 Firefly 开发运行时插件：从需求到可安装的 zip 包，一次做对。

## 何时使用

- 用户想给 Firefly 加新能力，且明确走插件形式（而不是改本体源码）
- 用户拿来的插件报错，需要排查（启用失败、工具不生效、弹窗异常）
- 用户询问插件怎么写、能做什么

不使用：改 Firefly 本体源码的普通开发任务；只是使用已装插件。
...
| 动态上下文插件 | `ctx.registerPromptProvider`：让流萤主动感知实时状态（天气/日程/番茄钟），无需用户开口问 |

---
name: firefly-exam-paper

description: Firefly 在 Learn 模式下的试卷能力：命题蓝图驱动出卷（知识点→能力→题型）、试卷与答案分离、批改评分、考试复盘。学科出题细则在 references/ 按需加载。
version: 1.2.1
autoInject: true
...
# Firefly Exam Paper（试卷能力）

出正式试卷的完整契约：先做命题蓝图，再按蓝图出卷；试卷与答案分离成两个文件，支持先考后批。文件操作安全原则已由 `firefly-obsidian-workspace` 定义，无需重复。

## 何时触发

- 用户说"出一张试卷"、"来场模拟考"、"正式测验一下"
- 阶段性检验学习成果的成套测验
- 几道题的随堂练习、小测仍走 `exercises/`（firefly-learn-tutor 第 5 节），不走本流程

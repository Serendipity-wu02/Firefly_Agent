---
name: firefly-obsidian-workspace

description: Firefly 在 Learn 模式下操作 Obsidian Vault 的规范：目录约定、文件读取、按标题定位、创建与修改 Markdown 的安全原则。
version: 1.0.0
autoInject: true
effectKind: mutation
modes:
...
本 Skill 描述 Firefly 在 Learn 模式下如何操作绑定的 Obsidian Vault。常驻规则已由 `learn_system.md` 定义，本 Skill 只保留可执行的文件操作规范。
